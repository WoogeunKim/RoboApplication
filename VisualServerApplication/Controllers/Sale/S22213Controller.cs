...
{
    [RoutePrefix("s22213")]
    public class S22213Controller : ApiController
    {
        #region MyRegion
        //// GET api/<controller>
        ...
        {
            return Ok<IEnumerable<SaleVo>>(Properties.EntityMapper.QueryForList<SaleVo>("S22213SelectMstList", vo));
        }
        ...
        public async Task<IHttpActionResult> GetDtlInsert([FromBody] IList<SaleVo> voList)
        {
            try
            {
                ...
                if (voList.Count > 0)
                {
                    ...
                    {
                        Properties.EntityMapper.QueryForObject<int?>("ProcS22213", voList[0]);
                    }
                }
