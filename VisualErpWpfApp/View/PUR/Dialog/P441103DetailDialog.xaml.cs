...
{
    public partial class P441103DetailDialog : DXWindow
	{
        ...
        private string _title = "부자재 발주 등록";
        ...
        public P441103DetailDialog(PurVo Dao)
		{
			InitializeComponent();
       ...
       //         }
                ...
                using (HttpResponseMessage response = await SystemProperties.PROGRAM_HTTP.PostAsync("p441103/popup", new StringContent(JsonConvert.SerializeObject(this.orgDao), System.Text.Encoding.UTF8, "application/json")))
                {
                    if (response.IsSuccessStatusCode)
                    {
		...
		private async void OKButton_Click(object sender, RoutedEventArgs e)
		{
            try
            {
                ...
                {
                    int _Num = 0;
                    using (HttpResponseMessage response = await SystemProperties.PROGRAM_HTTP.PostAsync("p441103/dtl/i", new StringContent(JsonConvert.SerializeObject((saveItems.FindAll(x => x.isCheckd == true)).ToArray()), System.Text.Encoding.UTF8, "application/json")))
                    {
                        if (response.IsSuccessStatusCode)
                        {
                ...
                if (impItmQty)
                {
                    if (e.IsValid)
                    {
                        ...
                        if (!masterDomain.PUR_QTY.Equals((e.Value == null ? "" : e.Value.ToString())))
                        {
                            ...
                            {
                                //총 합계
                                masterDomain.TOT_AMT = Convert.ToDouble(masterDomain.PUR_AMT) + Convert.ToDouble(masterDomain.VAT_AMT);


                            }
                            catch (Exception)
                            {
                                masterDomain.TOT_AMT = 0;
                            }
                ...
                else if (inReqDt)
                {
                    if (e.IsValid)
                    {
                        ...
                        if (!masterDomain.IN_REQ_DT.Equals((e.Value == null ? "" : e.Value.ToString())))
                        {
                            ...
                            DateTime _IN_REQ_DT;
                            if (DateTime.TryParse(e.Value.ToString(), out _IN_REQ_DT))
                            {
                                //masterDomain.IN_REQ_DT = Convert.ToDateTime(e.Value.ToString()).ToString("yyyy-MM-dd");
                                masterDomain.IN_REQ_DT = _IN_REQ_DT.ToString("yyyy-MM-dd");
                            ...
                            {
                                e.ErrorType = DevExpress.XtraEditors.DXErrorProvider.ErrorType.Critical;
                                e.ErrorContent = "yyyy-MM-dd 형식이 맞지 않습니다";
                                e.SetError(e.ErrorContent, e.ErrorType);
                                return;
                            }
                        }
