...
{
    public sealed class I6614ViewModel : ViewModelBase, INotifyPropertyChanged
    {
        ...
        public I6614ViewModel() 
        {
            ...
            //{
            //    if (!INAUD_NM_LIST[x].CLSS_CD.StartsWith("I"))
            //    {
          ...
          public async void Refresh()
          {
              ...
              try
              {
                ...
                using (HttpResponseMessage response = await SystemProperties.PROGRAM_HTTP.PostAsync("i6614/mst", new StringContent(JsonConvert.SerializeObject(_param), System.Text.Encoding.UTF8, "application/json")))
                {
                    ...
                    //SelectMstList = invClient.I6614SelectMstList(_param);
              ...
              {
                  //DXSplashScreen.Close();
                  WinUIMessageBox.Show(eLog.Message, "[" + SystemProperties.PROGRAM_TITLE + "]품목입고현황", MessageBoxButton.OK, MessageBoxImage.Error, MessageBoxResult.None, MessageBoxOptions.None);
                  return;
              }
        ...
        public async void SYSTEM_CODE_VO()
        {
            ...
            using (HttpResponseMessage response = await SystemProperties.PROGRAM_HTTP.GetAsync("s131/dtl/" + Properties.Settings.Default.SettingChnl + "/" + "L-007"))
            {
                if (response.IsSuccessStatusCode)
                {
                    INAUD_NM_LIST = JsonConvert.DeserializeObject<IEnumerable<SystemCodeVo>>(await response.Content.ReadAsStringAsync()).Cast<SystemCodeVo>().ToList().Where<SystemCodeVo>(x => x.CLSS_CD.StartsWith("I")).ToList();
