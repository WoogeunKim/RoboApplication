...
{
    public sealed class M6682ViewModel : ViewModelBase, INotifyPropertyChanged
    {
        ...
        public async void Refresh()
        {
            ...
            {
                WinUIMessageBox.Show(eLog.Message, "[에러]금형도면이력관리", MessageBoxButton.OK, MessageBoxImage.Information, MessageBoxResult.None, MessageBoxOptions.None);
                return;
            }
        }
        ...
        [Command]
        public async void RvisRefresh(string _MOLD_CD = null)
        {
            ...
            //
            using (HttpResponseMessage response = await SystemProperties.PROGRAM_HTTP.PostAsync("m6682", new StringContent(JsonConvert.SerializeObject(new ManVo() { MOLD_NO = SelectedMenuItem.MOLD_NO, CHNL_CD = SystemProperties.USER_VO.CHNL_CD }), System.Text.Encoding.UTF8, "application/json")))
            {
                if (response.IsSuccessStatusCode)
                {
                    ...
                    if (SelectedDwgRvisViewList.Count > 0)
                    {
                        ...
                        if (string.IsNullOrEmpty(_MOLD_CD))
                        {
                            SelectedDwgRvisItem = SelectedDwgRvisViewList[0];
                        }
                        else
                        {
                            SelectedDwgRvisItem = SelectedDwgRvisViewList.Where(x => (x.MOLD_CD).Equals(_MOLD_CD)).LastOrDefault<ManVo>();
                        }
                    }
                    else
                    {
        ...
        public void NewDwgRvisContact()
        {
            ...
            dwgDialog = new M6682MasterDialog(new ManVo() { MOLD_NO = SelectedMenuItem.MOLD_NO });
            dwgDialog.Title = "금형도면이력관리 - 추가 [" + SelectedMenuItem.MOLD_NO + " / " + SelectedMenuItem.ITM_CD + " / " + SelectedMenuItem.ITM_NM + "]";
            ...
            if (isDialog)
            {
                RvisRefresh(dwgDialog.updateDao.MOLD_NO);
                //SearchRvisContact();
                //Refresh();
                //if (SelectedMenuViewList.Count > 0)
                //{
        ...
        public async void EditDwgRvisContact()
        {
            ...
            {
                MessageBoxResult result = WinUIMessageBox.Show("[" + SelectedDwgRvisItem.RN + "/" + SelectedDwgRvisItem.MOLD_NO + "]" + " 정말로 삭제 하시겠습니까?", "금형도면이력관리", MessageBoxButton.YesNo, MessageBoxImage.Question);
                if (result == MessageBoxResult.Yes)
                {
                    ...
                    using (HttpResponseMessage response = await SystemProperties.PROGRAM_HTTP.PostAsync("m6682/u", new StringContent(JsonConvert.SerializeObject(SelectedDwgRvisItem), System.Text.Encoding.UTF8, "application/json")))
                    {
                        if (response.IsSuccessStatusCode)
                        {
                            int _Num = 0;
                            ...
                            {
                                //실패
                                WinUIMessageBox.Show(resultMsg, "금형도면이력관리", MessageBoxButton.OK, MessageBoxImage.Error);
                                return;
                            }
                            RvisRefresh();

                            //성공
                            WinUIMessageBox.Show("삭제가 완료되었습니다.", "금형도면이력관리", MessageBoxButton.OK, MessageBoxImage.Information, MessageBoxResult.None, MessageBoxOptions.None);
                        }
                    }
