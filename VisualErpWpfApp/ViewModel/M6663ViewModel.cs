...
{
    public sealed class M6663ViewModel : ViewModelBase, INotifyPropertyChanged
    {
        private string title = "금형위치관리";
        ...
        private M6663DetailDialog masterDialog;
        private M6663DetailHistoryDialog historyDialog;
        ...
        public async void Refresh(string _ITM_CD = null)
        {
            ...
            try
            {
               ...
               // _param.AREA_NM = M_SL_AREA_NM.CLSS_DESC;
                ...
                using (HttpResponseMessage response = await SystemProperties.PROGRAM_HTTP.PostAsync("m6661", new StringContent(JsonConvert.SerializeObject(_param), System.Text.Encoding.UTF8, "application/json")))
                {
                    ...
                    //SearchMenuContact();
                    ////
                    //Title = "[사업장]" + M_SL_AREA_NM.CLSS_DESC + ",  [구분]" + M_ITM_GRP_NM.CLSS_DESC;
                    ...
                    if (SelectMstList.Count >= 1)
                    {
                        ...
                        {
                            SelectedMstItem = SelectMstList.Where(x => x.MOLD_NO.Equals(_ITM_CD)).LastOrDefault<ManVo>();
                        }
                    }
                    else
                    {
        ...
        private async void SelectMstDetail()
        {
            try
            {
                ...
                using (HttpResponseMessage response = await SystemProperties.PROGRAM_HTTP.PostAsync("s1144/dtl", new StringContent(JsonConvert.SerializeObject(new SystemCodeVo() { CHNL_CD = SystemProperties.USER_VO.CHNL_CD, ITM_CD = this.SelectedMstItem.MOLD_NO }), System.Text.Encoding.UTF8, "application/json")))
                {
                    if (response.IsSuccessStatusCode)
                    {
                    ...
                    Title = "[사업장]" + M_SL_AREA_NM.CLSS_DESC + ",  [금형번호]" + SelectedMstItem.MOLD_NO + ",  [품번]" + SelectedMstItem.ITM_CD + ",  [품명]" + SelectedMstItem.ITM_NM ;
        ...
        public void NewContact()
        {
            try
            {
                ...
                string itmCd = SelectedMstItem.MOLD_NO;
                masterDialog = new M6663DetailDialog(SelectedMstItem);
                masterDialog.Title = title + " - [사업장]" + SystemProperties.USER_VO.EMPE_PLC_CD + ", [금형번호]" + SelectedMstItem.MOLD_NO + ", [품번]" + SelectedMstItem.ITM_CD + ", [품명]" + SelectedMstItem.ITM_NM;
        ...
        public void HistoryContact()
        {
            try
            {
                ...
                historyDialog = new M6663DetailHistoryDialog(new SystemCodeVo() { CHNL_CD = SystemProperties.USER_VO.CHNL_CD, AREA_CD = SystemProperties.USER_VO.EMPE_PLC_NM, ITM_CD = SelectedMstItem.MOLD_NO });
                historyDialog.Title = "이력 관리" + " - [사업장]" + SystemProperties.USER_VO.EMPE_PLC_CD + ", [금형번호]" + SelectedMstItem.MOLD_NO + ", [품번]" + SelectedMstItem.ITM_CD + ", [품명]" + SelectedMstItem.ITM_NM;
