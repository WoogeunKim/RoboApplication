...
{
    public sealed class M6630ViewModel : ViewModelBase, INotifyPropertyChanged
    {
        ...
        private M6630MasterDialog masterDialog;

        

        //private ICommand _searchDialogCommand;

        public M6630ViewModel() 
        {
        ...
        public async void NewContact()
        {
            try
            {
                masterDialog = new M6630MasterDialog(new ManVo());
        ...
        public async void EditContact()
        {
            try
            {
                ...
                masterDialog = new M6630MasterDialog(SelectedMstItem);
                masterDialog.Title = SelectedMstItem.PROD_PLN_NO + " / " + SelectedMstItem.CRE_DT;
                masterDialog.WindowStartupLocation = WindowStartupLocation.CenterScreen;
                masterDialog.Owner = Application.Current.MainWindow;
                masterDialog.BorderEffect = BorderEffect.Default;
                //masterDialog.BorderEffectActiveColor = new SolidColorBrush(Color.FromRgb(255, 128, 0));
        ...
        public async void DelContact()
        {
            try
            {
                ManVo delDao = this.SelectedMstItem;
                if (delDao != null)
                {
                    MessageBoxResult result = WinUIMessageBox.Show("[" + delDao.PROD_PLN_NO + "/" + delDao.CRE_DT + "] 정말로 삭제 하시겠습니까?", _title, MessageBoxButton.YesNo, MessageBoxImage.Question);
                    if (result == MessageBoxResult.Yes)
                    {
                        delDao.DELT_FLG = "Y";
                        delDao.UPD_USR_ID = SystemProperties.USER;
                        delDao.CHNL_CD = SystemProperties.USER_VO.CHNL_CD;
                        using (HttpResponseMessage response = await SystemProperties.PROGRAM_HTTP.PostAsync("m6630/mst/u", new StringContent(JsonConvert.SerializeObject(delDao), System.Text.Encoding.UTF8, "application/json")))
                        {
        ...
        private async void SearchDetailItem()
        {
            ...
            //
            using (HttpResponseMessage response = await SystemProperties.PROGRAM_HTTP.PostAsync("m6630/dtl", new StringContent(JsonConvert.SerializeObject(SelectedMstItem), System.Text.Encoding.UTF8, "application/json")))
            {
                if (response.IsSuccessStatusCode)
                {
