...
{
    public partial class S1146MasterDialog : DXWindow
    {
        ...
        public S1146MasterDialog(SystemCodeVo Dao)
        {
            InitializeComponent();
            //
            ...
            {
                this.isEdit = true;
                this.text_ITM_NM.IsEnabled = false;
            }
            else
            {
                Dao.ST_APLY_DT = System.DateTime.Now.ToString("yyyy-MM-dd");
        ...
        private async void OKButton_Click(object sender, RoutedEventArgs e)
        {
            if (ValueCheckd() == true)
            {
                ...
                if (isEdit == false)
                {
                    ...
                    //{
                    using (HttpResponseMessage response = await SystemProperties.PROGRAM_HTTP.PostAsync("S1146/i", new StringContent(JsonConvert.SerializeObject(this.updateVo), System.Text.Encoding.UTF8, "application/json")))
                    {
                        if (response.IsSuccessStatusCode)
                        {
                ...
                {
                    using (HttpResponseMessage response = await SystemProperties.PROGRAM_HTTP.PostAsync("S1146/u", new StringContent(JsonConvert.SerializeObject(this.updateVo), System.Text.Encoding.UTF8, "application/json")))
                    {
                        if (response.IsSuccessStatusCode)
                        {
        ...
        public SystemCodeVo getDomain()
        {
            ...
            ItemCodeVo itemDao = this.combo_ITM_NM.SelectedItem as ItemCodeVo;
            if (itemDao != null)
            {
            ...
            Dao.ST_APLY_DT = Convert.ToDateTime(this.text_FM_DT.Text).ToString("yyyy-MM-dd");
            Dao.END_APLY_DT = "2999-12-31";
        ...
        public async void SYSTEM_CODE_VO()
        {
            ...
            using (HttpResponseMessage response = await SystemProperties.PROGRAM_HTTP.PostAsync("s141/mini", new StringContent(JsonConvert.SerializeObject(new SystemCodeVo() { CHNL_CD = SystemProperties.USER_VO.CHNL_CD, DELT_FLG = "N", ITM_GRP_CLSS_CD = "G" /*N1ST_ITM_GRP_CD = SelectedMasterItem.N1ST_ITM_GRP_CD*/ }), System.Text.Encoding.UTF8, "application/json")))
            {
                if (response.IsSuccessStatusCode)
                {
                    this.combo_ITM_NM.ItemsSource = JsonConvert.DeserializeObject<IEnumerable<ItemCodeVo>>(await response.Content.ReadAsStringAsync()).Cast<ItemCodeVo>().ToList();
                }
            }
        }
