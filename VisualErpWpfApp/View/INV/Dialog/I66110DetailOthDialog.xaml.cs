...
{
    public partial class I66110DetailOthDialog : DXWindow
    {
        //private static InvServiceClient invClient = SystemProperties.InvClient;
        private InvVo orgVo;
        private string _title = "품목 기타 관리";

        public I66110DetailOthDialog(InvVo vo)
        {
            InitializeComponent();
        ...
        private async void searchItem()
        {
            try
            {
                ...
                //    return;
                //}
                using (HttpResponseMessage response = await SystemProperties.PROGRAM_HTTP.PostAsync("i66110/oth", new StringContent(JsonConvert.SerializeObject(vo), System.Text.Encoding.UTF8, "application/json")))
                {
        ...
        private async void OKButton_Click(object sender, RoutedEventArgs e)
        {
            ...
            List<InvVo> saveList = checkList.Where<InvVo>(x => x.isCheckd).ToList<InvVo>(); //new List<InvVo>();
            ...
            if (result == MessageBoxResult.Yes)
            {
                ...
                //{
                    int _Num = 0;
                    using (HttpResponseMessage response = await SystemProperties.PROGRAM_HTTP.PostAsync("i66110/mst/i", new StringContent(JsonConvert.SerializeObject(saveList.ToArray()), System.Text.Encoding.UTF8, "application/json")))
                    {
                        if (response.IsSuccessStatusCode)
                        {
        ...
        private void GridColumn_Validate(object sender, DevExpress.Xpf.Grid.GridCellValidationEventArgs e)
        {
            try
            {
                ...
                bool batchCd = (e.Column.FieldName.ToString().Equals("BATCH_CD") ? true : false);
                bool mstLotNo = (e.Column.FieldName.ToString().Equals("MST_LOT_NO") ? true : false);
                bool serNo = (e.Column.FieldName.ToString().Equals("SER_NO") ? true : false);
                bool expNo = (e.Column.FieldName.ToString().Equals("EXP_NO") ? true : false);
                ...
                else if (batchCd)
                {
                    if (e.IsValid)
                    {
                        if (string.IsNullOrEmpty(masterDomain.BATCH_CD))
                        {
                            masterDomain.BATCH_CD = "";
                        }
                        //
                        if (!masterDomain.BATCH_CD.Equals((e.Value == null ? "" : e.Value.ToString())))
                        {
                            masterDomain.BATCH_CD = e.Value.ToString();
                            masterDomain.isCheckd = true;
                            this.OKButton.IsEnabled = true;
                        }
                    }
                ...
                else if (mstLotNo)
                {
                    if (e.IsValid)
                    {
                        if (string.IsNullOrEmpty(masterDomain.MST_LOT_NO))
                        {
                            masterDomain.MST_LOT_NO = "";
                        }
                        //
                        if (!masterDomain.MST_LOT_NO.Equals((e.Value == null ? "" : e.Value.ToString())))
                        {
                            masterDomain.MST_LOT_NO = e.Value.ToString();
                            masterDomain.isCheckd = true;
                            this.OKButton.IsEnabled = true;
                        }
                    }
                ...
                else if (serNo)
                {
                    if (e.IsValid)
                    {
                        if (string.IsNullOrEmpty(masterDomain.SER_NO))
                        {
                            masterDomain.SER_NO = "";
                        }
                        //
                        if (!masterDomain.SER_NO.Equals((e.Value == null ? "" : e.Value.ToString())))
                        {
                            masterDomain.SER_NO = e.Value.ToString();
                            masterDomain.isCheckd = true;
                            this.OKButton.IsEnabled = true;
                        }
                    }
                ...
                else if (expNo)
                {
                    if (e.IsValid)
                    {
                        if (string.IsNullOrEmpty(masterDomain.EXP_NO))
                        {
                            masterDomain.EXP_NO = "";
                        }
                        //
                        if (!masterDomain.EXP_NO.Equals((e.Value == null ? "" : e.Value.ToString())))
                        {
                            masterDomain.EXP_NO = e.Value.ToString();
                            masterDomain.isCheckd = true;
                            this.OKButton.IsEnabled = true;
                        }
                    }
