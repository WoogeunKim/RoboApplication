...
namespace AquilaErpWpfApp3.View.M.Dialog
{
    ...
    /// </summary>
    public partial class M6631Master2Dialog : DXWindow
    {
        ...
        public M6631Master2Dialog(ManVo Dao)
        {
            InitializeComponent();
        ...
        private async void OKButton_Click(object sender, RoutedEventArgs e)
        {
            if (ValueCheckd())
            {
                ...
                if (isEdit == false)
                {
                    ...
                    MessageBoxResult result = WinUIMessageBox.Show("[" + this.updateDao.MIX_WEIH_VAL + "Kg] 생성 하시겠습니까?", "[추가] 칭량 작업 계획", MessageBoxButton.YesNo, MessageBoxImage.Question);
        ...
        private ManVo getDomain()
        {
            ...
            Dao.MIX_WEIH_VAL = Convert.ToDecimal(this.text_TOT_WEIH_VAL.Text);
            Dao.TOT_WEIH_VAL = Convert.ToDecimal(this.text_TOT_WEIH_VAL.Text);
            ...
            try
            {
                ...
                using (HttpResponseMessage response = await SystemProperties.PROGRAM_HTTP.GetAsync("s131/dtl/" + Properties.Settings.Default.SettingChnl + "/" + "L-001"))
                {
                    if (response.IsSuccessStatusCode)
                    {
                        ...
                        this.combo_ITM_GRP_CLSS_CD.SelectedItem = (this.combo_ITM_GRP_CLSS_CD.ItemsSource as List<SystemCodeVo>).Where(x => x.CLSS_CD.Equals("S")).FirstOrDefault<SystemCodeVo>();
                    }

                    //
                    using (HttpResponseMessage responseX = await SystemProperties.PROGRAM_HTTP.PostAsync("s133", new StringContent(JsonConvert.SerializeObject(new SystemCodeVo() { DELT_FLG = "N", ITM_GRP_CLSS_CD = "S", CRE_USR_ID = "", CHNL_CD = SystemProperties.USER_VO.CHNL_CD }), System.Text.Encoding.UTF8, "application/json")))
                    {
