...
namespace AquilaErpWpfApp3.View.S.Dialog
{
    ...
    /// </summary>
    public partial class S11479MasterDialog : DXWindow
    {
        ...
        public S11479MasterDialog(SystemCodeVo Dao)
        {
            ...
            else
            {
                ...
                this.isEdit = false;
                copyDao.DELT_FLG = "사용";
                copyDao.S_CRE_DT = System.DateTime.Now.ToString("yyyy-MM-dd");
                copyDao.S_DUE_DT = System.DateTime.Now.ToString("yyyy-MM-dd");
            }
            ...
            try
            {
                if (ValueCheckd())
                {
                    ...
                    if (isEdit == false)
                    {
                        ...
                        using (HttpResponseMessage response = await SystemProperties.PROGRAM_HTTP.PostAsync("s11479/mst/i", new StringContent(JsonConvert.SerializeObject(this.updateDao), System.Text.Encoding.UTF8, "application/json")))
                        {
                    ...
                    else
                    {
                        ...
                        using (HttpResponseMessage response = await SystemProperties.PROGRAM_HTTP.PostAsync("s11479/mst/u", new StringContent(JsonConvert.SerializeObject(this.updateDao), System.Text.Encoding.UTF8, "application/json")))
                        {
                            if (response.IsSuccessStatusCode)
                            {
                                string result = await response.Content.ReadAsStringAsync();
