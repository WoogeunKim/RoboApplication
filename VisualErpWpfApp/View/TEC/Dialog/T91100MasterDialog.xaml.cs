...
namespace AquilaErpWpfApp3.View.TEC.Dialog
{
    ...
    /// </summary>
    public partial class T91100MasterDialog : DXWindow
    {
        ...
        public T91100MasterDialog(IList<TecVo> _chekList)
        {
            InitializeComponent();
            //
            //this.orgDao = Dao;
        ...
        {
            this._badList.Add(new TecVo() { BAD_QTY = 0, GRP_SER_NO = this.ChekList[0].GRP_SER_NO, ROUT_CD = this.ChekList[0].ROUT_CD, INAUD_TMP_NO = this.ChekList[0].INAUD_TMP_NO, INAUD_TMP_SEQ = this.ChekList[0].INAUD_TMP_SEQ, CRE_USR_ID = SystemProperties.USER_VO.USR_ID, UPD_USR_ID = SystemProperties.USER_VO.USR_ID });
        ...
        private async void OKButton_Click(object sender, RoutedEventArgs e)
        {
            if (ValueCheckd())
            {
                ...
                //
                //
                using (HttpResponseMessage response = await SystemProperties.PROGRAM_HTTP.PostAsync("t91100/m", new StringContent(JsonConvert.SerializeObject(this.ChekList), System.Text.Encoding.UTF8, "application/json")))
                {
                    if (response.IsSuccessStatusCode)
                    {
                        string result = await response.Content.ReadAsStringAsync();
                        ...
                        if (this._badList.Count > 0)
                        {
                            //
                            //불량 유형 (삭제 -> 저장)
                            using (HttpResponseMessage response_Y = await SystemProperties.PROGRAM_HTTP.PostAsync("t91100/bad/m", new StringContent(JsonConvert.SerializeObject(this._badList), System.Text.Encoding.UTF8, "application/json")))
                            {
        ...
        public async void SYSTEM_CODE_VO()
        {
            ...
            //
            using (HttpResponseMessage response = await SystemProperties.PROGRAM_HTTP.PostAsync("t91100/bad", new StringContent(JsonConvert.SerializeObject(this.ChekList[0]), System.Text.Encoding.UTF8, "application/json")))
            {
                if (response.IsSuccessStatusCode)
                {
                ...
                if (badQty)
                {
                    if (e.IsValid)
                    {
                        ...
                        if (!masterDomain.BAD_QTY.Equals((e.Value == null ? "" : e.Value.ToString())))
                        {
                            ...
                            masterDomain.SCM_PUR_ITM_CD = this.ChekList[0].SCM_PUR_ITM_CD;

                            masterDomain.INAUD_TMP_NO = this.ChekList[0].INAUD_TMP_NO;
                            masterDomain.INAUD_TMP_SEQ = this.ChekList[0].INAUD_TMP_SEQ;
