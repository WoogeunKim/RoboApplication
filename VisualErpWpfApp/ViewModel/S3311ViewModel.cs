...
namespace AquilaErpWpfApp3.ViewModel
{
    public sealed class S3311ViewModel : ViewModelBase, INotifyPropertyChanged
    {
        ...
        [Command]
        public async void Refresh(string _FM_DT_CO_NM = null)
        {
            try
            {
                ...
                using (HttpResponseMessage response = await SystemProperties.PROGRAM_HTTP.PostAsync("s3311/mst", new StringContent(JsonConvert.SerializeObject(new SaleVo() { FM_DT = (StartDt).ToString("yyyy-MM-dd"), TO_DT = (EndDt).ToString("yyyy-MM-dd"), SL_AREA_CD = M_SL_AREA_NM.CLSS_CD, CHNL_CD = SystemProperties.USER_VO.CHNL_CD }), System.Text.Encoding.UTF8, "application/json")))
                {
                    ...
                    if (SelectMstList.Count >= 1)
                    {
                        ...
                        if (string.IsNullOrEmpty(_FM_DT_CO_NM))
                        {
                            SelectedMstItem = SelectMstList[0];
                        }
                        else
                        {
                            SelectedMstItem = SelectMstList.Where(x => (x.FM_DT + x.CO_NM).Equals(_FM_DT_CO_NM)).LastOrDefault<SaleVo>();
                        }
                    }
                    else
                    {
        ...
        {
            masterDialog = new S3311MasterDialog(new SaleVo() { DELT_FLG = "N", SL_AREA_CD = M_SL_AREA_NM.CLSS_CD, SL_AREA_NM = M_SL_AREA_NM.CLSS_DESC, FM_DT = System.DateTime.Now.ToString("yyyy-MM-dd") });
        ...
        public async void DelContact()
        {
            ...
            try
            {
                ...
                if (delDao != null)
                {
                    MessageBoxResult result = WinUIMessageBox.Show("[" + delDao.FM_DT + " / " + delDao.CO_NM + "]" + " 정말로 삭제 하시겠습니까?", "[삭제]" + _title, MessageBoxButton.YesNo, MessageBoxImage.Question);
