...
{
    public sealed class S2222ViewModel : ViewModelBase, INotifyPropertyChanged
    {
        private string title = "상계처리";
        ...
        [Command]
        public async void Refresh(string _CLT_BIL_DELT_NO = null)
        {
            try
            {
                ...
                using (HttpResponseMessage response = await SystemProperties.PROGRAM_HTTP.PostAsync("s2222/mst", new StringContent(JsonConvert.SerializeObject(_param), System.Text.Encoding.UTF8, "application/json")))
                {
