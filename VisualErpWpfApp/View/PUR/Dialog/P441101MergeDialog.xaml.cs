...
{
    public partial class P441101MergeDialog : DXWindow
    {
        ...
        public P441101MergeDialog(IList<PurVo> _selectMstList)
        {
            InitializeComponent();
        ...
        private async void ApplyButton_Click(object sender, RoutedEventArgs e)
        {
            try
            {
                if (this.ViewGridMst.SelectedItems.Count > 0)
                {
                    ...
                    //
                    using (HttpResponseMessage response = await SystemProperties.PROGRAM_HTTP.PostAsync("p441101/marge", new StringContent(JsonConvert.SerializeObject(_param), System.Text.Encoding.UTF8, "application/json")))
                    {
        ...
        private async void OKButton_Click(object sender, RoutedEventArgs e)
        {
            ...
            if (this.ViewGridDtl.SelectedItems.Count > 0)
            {
                ...
                if (WinUIMessageBox.Show("(" + _gbn + ")  자동 발주 하시겠습니까?", _title, MessageBoxButton.YesNo, MessageBoxImage.Question) == MessageBoxResult.Yes)
                {
                    ...
                    //
                    int _Num = 0;
                    using (HttpResponseMessage response = await SystemProperties.PROGRAM_HTTP.PostAsync("p441101/marge/m", new StringContent(JsonConvert.SerializeObject(_purList), System.Text.Encoding.UTF8, "application/json")))
                    {
                        if (response.IsSuccessStatusCode)
                        {
