...
{
    public partial class M6710 : UserControl
    {
        private string title = "실적등록";
        public M6710()
        {
            DataContext = new M6710ViewModel();
            //
            InitializeComponent();
            //
                ...
                else if (wrkStDt)
                {
                    if (e.IsValid)
                    {
                        ...
                        //if (!masterDomain.WRK_ST_DT.Equals((e.Value == null ? "" : e.Value.ToString())))
                        {
                            ...
                            else if (Convert.ToDateTime(masterDomain?.WRK_END_DT) < Convert.ToDateTime(masterDomain?.WRK_ST_DT))
                            {
                                e.ErrorType = DevExpress.XtraEditors.DXErrorProvider.ErrorType.Critical;
                                e.ErrorContent = "[" + e.Value.ToString() + "] (HH:MM) 시작시간이 종료시간보다 큽니다";
                                e.SetError(e.ErrorContent, e.ErrorType);
                                masterDomain.WRK_HRS = 0;
                                return;
                            }
                            else
                            {
                                    ...
                                    if (response.IsSuccessStatusCode)
                                    {
                                        ...
                                        using (HttpResponseMessage response_x = await SystemProperties.PROGRAM_HTTP.PostAsync("m6710/mst/r/u", new StringContent(JsonConvert.SerializeObject(masterDomain), System.Text.Encoding.UTF8, "application/json")))
                                        {
                                            if (response_x.IsSuccessStatusCode)
                                            {
                                                ...
                                                //_tmpVo = JsonConvert.DeserializeObject<ManVo>(await response.Content.ReadAsStringAsync());

                                                //masterDomain.WRK_HRS = _tmpVo.WRK_HRS;
                                            }
                                        }
                                ...
                                using (HttpResponseMessage response = await SystemProperties.PROGRAM_HTTP.PostAsync("m6710/mst/u", new StringContent(JsonConvert.SerializeObject(masterDomain), System.Text.Encoding.UTF8, "application/json")))
                                {
                                    if (response.IsSuccessStatusCode)
                                    {
                                        ...
                                        using (HttpResponseMessage response_x = await SystemProperties.PROGRAM_HTTP.PostAsync("m6710/mst/r/u", new StringContent(JsonConvert.SerializeObject(masterDomain), System.Text.Encoding.UTF8, "application/json")))
                                        {
                                            if (response_x.IsSuccessStatusCode)
                                            {
                                                //_tmpVo = JsonConvert.DeserializeObject<string>(await response.Content.ReadAsStringAsync());
