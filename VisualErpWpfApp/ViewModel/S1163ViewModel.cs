...
{
    public sealed class S1163ViewModel : ViewModelBase, INotifyPropertyChanged
    {
        private string title = "결근/지각자 현황";
        ...
        public async void Refresh(string EMPE_ID = null)
        {
            try
            {
                using (HttpResponseMessage response = await SystemProperties.PROGRAM_HTTP.PostAsync("s1162", new StringContent(JsonConvert.SerializeObject(new SystemCodeVo() { FM_DT = (StartDt).ToString("yyyy-MM-dd"), TO_DT = (EndDt).ToString("yyyy-MM-dd"), CHNL_CD = SystemProperties.USER_VO.CHNL_CD , A_WRK_RSN_CD = new string[]{ "B004", "B005" } }), System.Text.Encoding.UTF8, "application/json")))
                {
