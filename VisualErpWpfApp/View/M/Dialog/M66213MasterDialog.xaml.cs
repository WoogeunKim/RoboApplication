...
namespace AquilaErpWpfApp3.View.M.Dialog
{
    ...
    /// </summary>
    public partial class M66213MasterDialog : DXWindow
    {
        ...
        public M66213MasterDialog(ManVo Dao, bool IsEdit)
        {
            InitializeComponent();
            //
            this.orgDao = Dao;
        ...
        private ManVo getDomain()
        {
            ...
            //
            Dao.MSEQ = this.orgDao.MSEQ;
            Dao.UPSEQ = this.orgDao.UPSEQ;
            Dao.SEQ = this.orgDao.SEQ;
            ...
            ManVo _mold_no = this.combo_MOLD_NO.SelectedItem as ManVo;
            if (_mold_no != null)
            {
                Dao.MOLD_CD = _mold_no.MOLD_NO;
                Dao.WEIH_SEQ = _mold_no.WEIH_SEQ;
                //Dao.WEIH_SEQ = this.text_MOLD_SEQ.Text;
            }



            ItemCodeVo _mtrl_cd = this.combo_MTRL_CD.SelectedItem as ItemCodeVo;
            if (_mtrl_cd != null)
            {
                Dao.MTRL_CD = _mtrl_cd.ITM_CD;
            }
        ...
        public async void SYSTEM_CODE_VO()
        {
            ...
            using (HttpResponseMessage response = await SystemProperties.PROGRAM_HTTP.PostAsync("m66213/tree", new StringContent(JsonConvert.SerializeObject(this.orgDao), System.Text.Encoding.UTF8, "application/json")))
            {
                if (response.IsSuccessStatusCode)
                {
                    ...
                    _tmpList.Add(new ManVo() { CMPO_CD = this.orgDao.ROUT_ITM_CD, CMPO_NM = this.orgDao.ITM_NM , GBN = ""});
            ...
            using (HttpResponseMessage response = await SystemProperties.PROGRAM_HTTP.PostAsync("m6661", new StringContent(JsonConvert.SerializeObject(new ManVo() { DELT_FLG = "N",  CHNL_CD = SystemProperties.USER_VO.CHNL_CD }), System.Text.Encoding.UTF8, "application/json")))
            {
                if (response.IsSuccessStatusCode)
                {
                    this.combo_MOLD_NO.ItemsSource = JsonConvert.DeserializeObject<IEnumerable<ManVo>>(await response.Content.ReadAsStringAsync()).Cast<ManVo>().ToList();
                    if (string.IsNullOrEmpty(this.orgDao.GBN) == false)
                    {
                        this.combo_MOLD_NO.SelectedItem = (this.combo_MOLD_NO.ItemsSource as IEnumerable<ManVo>).Where<ManVo>(w => w.GBN.Equals(this.orgDao.GBN)).FirstOrDefault<ManVo>();
                    }
                }
            }

            //원재료
            using (HttpResponseMessage response = await SystemProperties.PROGRAM_HTTP.PostAsync("s141/mini", new StringContent(JsonConvert.SerializeObject(new SystemCodeVo() { ITM_GRP_CLSS_CD = "M", DELT_FLG = "N", CHNL_CD = SystemProperties.USER_VO.CHNL_CD }), System.Text.Encoding.UTF8, "application/json")))
            {
                if (response.IsSuccessStatusCode)
                {
                    this.combo_MTRL_CD.ItemsSource = JsonConvert.DeserializeObject<IEnumerable<ItemCodeVo>>(await response.Content.ReadAsStringAsync()).Cast<ItemCodeVo>().ToList();
                    if (string.IsNullOrEmpty(this.orgDao.MTRL_CD) == false)
                    {
                        this.combo_MTRL_CD.SelectedItem = (this.combo_MTRL_CD.ItemsSource as IEnumerable<ItemCodeVo>).Where<ItemCodeVo>(w => w.ITM_CD.Equals(this.orgDao.MTRL_CD)).FirstOrDefault<ItemCodeVo>();
                    }
                }
            }
