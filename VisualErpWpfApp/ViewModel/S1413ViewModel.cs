...
{
    public sealed class S1413ViewModel : ViewModelBase, INotifyPropertyChanged
    {
        ...
        public async void Refresh()
        {
            try
            {
                ...
                //톤 단위
                _param.BAR_CLSS_CD = this.M_TON_NM.CLSS_CD;
                //채널
                _param.CHNL_CD = SystemProperties.USER_VO.CHNL_CD;

                ////
                //SelectMstList = saleOrderClient.S2222SelectMstList(_param);
                using (HttpResponseMessage response = await SystemProperties.PROGRAM_HTTP.PostAsync("s1413/mst", new StringContent(JsonConvert.SerializeObject(_param), System.Text.Encoding.UTF8, "application/json")))
                {
                    ...
                    if (SelectMstList.Count >= 1)
                    {
                        ...
                        //else
                        //{
                            //SelectedMstItem = SelectMstList.Where(x => x.CLT_BIL_DELT_NO.Equals(_CLT_BIL_DELT_NO)).LastOrDefault<SaleVo>();
                        //}
                    }
                    else
                    {
        ...
        //톤 단위
        //private Dictionary<string, string> _AreaMap = new Dictionary<string, string>();
        private IList<SystemCodeVo> _TonCd = new List<SystemCodeVo>();
        public IList<SystemCodeVo> TonList
        {
            get { return _TonCd; }
            set { SetProperty(ref _TonCd, value, () => TonList); }
        }

        //톤 단위
        private SystemCodeVo _M_TON_NM;
        public SystemCodeVo M_TON_NM
        {
            get { return _M_TON_NM; }
            set { SetProperty(ref _M_TON_NM, value, () => M_TON_NM); }
        }
        ...
        //{
        //    masterDialog = new S2222MasterDialog(new SaleVo() { AREA_NM = M_SL_AREA_NM.CLSS_DESC, AREA_CD = M_SL_AREA_NM.CLSS_CD });
        ...
        //    if (isDialog)
        //    {
        //        Refresh(masterDialog.CLT_BIL_DELT_NO);
        ...
        //        //        {
        //        //            if ((masterDialog.CLT_BIL_DELT_NO).Equals(SelectMstList[x].CLT_BIL_DELT_NO))
        //        //            {
        ...
        //    masterDialog = new S2222MasterDialog(SelectedMstItem);
        //    masterDialog.Title = title + " - 수정(" + SelectedMstItem.CLT_BIL_DELT_NO + ")";
        //    masterDialog.Owner = Application.Current.MainWindow;
        //    masterDialog.BorderEffect = BorderEffect.Default;
        //    masterDialog.BorderEffectActiveColor = new SolidColorBrush(Color.FromRgb(255, 128, 0));
        //    masterDialog.BorderEffectInactiveColor = new SolidColorBrush(Color.FromRgb(255, 170, 170));
        ...
        //    if (SelectedMstItem != null)
        //    {
        //        MessageBoxResult result = WinUIMessageBox.Show("[" + SelectedMstItem.CLT_BIL_DELT_NO + "] 정말로 삭제 하시겠습니까?", "[삭제]" + title, MessageBoxButton.YesNo, MessageBoxImage.Question);
        //        if (result == MessageBoxResult.Yes)
        //        {
        //            try
        //            {
        //                using (HttpResponseMessage response = await SystemProperties.PROGRAM_HTTP.PostAsync("s2222/mst/d", new StringContent(JsonConvert.SerializeObject(SelectedMstItem), System.Text.Encoding.UTF8, "application/json")))
        //                {
        //                    if (response.IsSuccessStatusCode)
        //                    {
        //                        int _Num = 0;
        ...
        //                return;
        //            }
        //            //saleOrderClient.S2222DeleteMst(SelectedMstItem);
        ...
        public async void SYSTEM_CODE_VO()
        {
            ...
            //TON 단위
            using (HttpResponseMessage response = await SystemProperties.PROGRAM_HTTP.GetAsync("s131/dtl/" + Properties.Settings.Default.SettingChnl + "/" + "S-060"))
            {
                if (response.IsSuccessStatusCode)
                {
                    TonList = JsonConvert.DeserializeObject<IEnumerable<SystemCodeVo>>(await response.Content.ReadAsStringAsync()).Cast<SystemCodeVo>().ToList();
                    if (TonList.Count > 0)
                    {
                        M_TON_NM = TonList[0];
                    }
