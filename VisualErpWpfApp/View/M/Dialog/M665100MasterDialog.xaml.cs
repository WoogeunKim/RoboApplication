...
namespace AquilaErpWpfApp3.M.View.Dialog
{
    ...
    public partial class M665100MasterDialog : DXWindow
    {
        ...
        public M665100MasterDialog(ManVo Dao, bool _isEdit)
        {
            InitializeComponent();
            //
            this.orgDao = Dao;
            ...
            {
                this.isEdit = true;
                //this.text_WKY_YRMON.IsReadOnly = true;
                //this.text_WK.IsReadOnly = true;
                //this.text_WKY_YRMON.IsEnabled = true;
                //this.text_WK.IsEnabled = true;
            }
            else
            {
                this.isEdit = false;
            ...
            //this.text_WKY_YRMON.Text = copyDao.WKY_YRMON;
            //this.text_WK.Text = copyDao.WK;
        ...
        private async void OKButton_Click(object sender, RoutedEventArgs e)
        {
            if (ValueCheckd())
            {
                ...
                if (isEdit == false)
                {
                    ...
                    using (HttpResponseMessage response = await SystemProperties.PROGRAM_HTTP.PostAsync("m665100/i", new StringContent(JsonConvert.SerializeObject(this.updateDao), System.Text.Encoding.UTF8, "application/json")))
                    {
                ...
                {
                    this.updateDao = getDomain();
                    using (HttpResponseMessage response = await SystemProperties.PROGRAM_HTTP.PostAsync("m665100/u", new StringContent(JsonConvert.SerializeObject(this.updateDao), System.Text.Encoding.UTF8, "application/json")))
                    {
                        if (response.IsSuccessStatusCode)
                        {
                            string result = await response.Content.ReadAsStringAsync();
        ...
        {
            ManVo Dao = new ManVo();
            Dao.FM_DT = (this.ViewGridMst.SelectedItem as ManVo).FM_DT;
            Dao.TO_DT = (this.ViewGridMst.SelectedItem as ManVo).TO_DT;

            Dao.WKY_YRMON = this.text_WKY_YRMON.Text;
            Dao.MM_RMK =  this.orgDao.WKY_YRMON;

            Dao.WK =  Convert.ToInt32( this.text_WK.Text);
            Dao.MM_12 = "" + this.orgDao.WK;
