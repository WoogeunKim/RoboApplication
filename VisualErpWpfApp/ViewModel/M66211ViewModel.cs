...
{
    //표준 공정 관리
    public sealed class M66211ViewModel : ViewModelBase, INotifyPropertyChanged
    {
        private string _title = "센서 관리";
        ...
        //
        private M66211MasterDialog masterDialog;

       
        public M66211ViewModel() 
        {
            Refresh();
        }

        [Command]
       public async void Refresh(string _SENS_CD = null)
        {
            try
            {
                using (HttpResponseMessage response = await SystemProperties.PROGRAM_HTTP.PostAsync("m66211", new StringContent(JsonConvert.SerializeObject(new ManVo() { CHNL_CD = SystemProperties.USER_VO.CHNL_CD }), System.Text.Encoding.UTF8, "application/json")))
                {
                    ...
                    if (SelectedMasterViewList.Count > 0)
                    {
                        ...
                        if (string.IsNullOrEmpty(_SENS_CD))
                        {
                            SelectedMasterItem = SelectedMasterViewList[0];
                        }
                        else
                        {
                            SelectedMasterItem = SelectedMasterViewList.Where(x => x.SENS_CD.Equals(_SENS_CD)).LastOrDefault<ManVo>();
                        }
                    }
                    else
                    {
        ...
        {
            masterDialog = new M66211MasterDialog(new ManVo());
            ...
            if (isDialog)
            {
                Refresh(masterDialog.resultDomain.SENS_CD);
        ...
        public void EditMasterContact()
        {
            ...
            if (editDao != null)
            {
                masterDialog = new M66211MasterDialog(editDao);
        ...
        public async void DelMasterContact()
        {
            try
            {
                ...
                if (delDao != null)
                {
                    MessageBoxResult result = WinUIMessageBox.Show(delDao.SENS_CD + " 정말로 삭제 하시겠습니까?", _title, MessageBoxButton.YesNo, MessageBoxImage.Question);
                    if (result == MessageBoxResult.Yes)
                    {
                        using (HttpResponseMessage response = await SystemProperties.PROGRAM_HTTP.PostAsync("m66211/d", new StringContent(JsonConvert.SerializeObject(delDao), System.Text.Encoding.UTF8, "application/json")))
                        {
                            if (response.IsSuccessStatusCode)
                            {
                                int _Num = 0;
