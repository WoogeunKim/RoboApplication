...
namespace AquilaErpWpfApp3.View.M.Dialog
{
    ...
    /// </summary>
    public partial class M665101MasterDialog : DXWindow
    {
        ...
        public M665101MasterDialog(ManVo Dao)
        {
            InitializeComponent();
        ...
        public async void SYSTEM_CODE_VO()
        {
            try
            {
                ...
                if (isEdit == false)
                {
                    //수주 번호
                    //
                    using (HttpResponseMessage response = await SystemProperties.PROGRAM_HTTP.PostAsync("M665101/popup", new StringContent(JsonConvert.SerializeObject(new ManVo() { FM_DT = "", CLZ_FLG = "N", UPD_USR_ID = SystemProperties.USER, CHNL_CD = SystemProperties.USER_VO.CHNL_CD, A_ROUT_CD = new string[] { "IN" } }), System.Text.Encoding.UTF8, "application/json")))
                    {
                        if (response.IsSuccessStatusCode)
                        {
                ...
                {
                    using (HttpResponseMessage response = await SystemProperties.PROGRAM_HTTP.PostAsync("M665101/popup", new StringContent(JsonConvert.SerializeObject(new ManVo() { PROD_ORD_NO = this.orgDao.PROD_ORD_NO, UPD_USR_ID = SystemProperties.USER, CHNL_CD = SystemProperties.USER_VO.CHNL_CD, A_ROUT_CD = new string[] { "IN" } }), System.Text.Encoding.UTF8, "application/json")))
                    {
                        if (response.IsSuccessStatusCode)
                        {
                            this.combo_PROD_ORD_NO.ItemsSource = JsonConvert.DeserializeObject<IEnumerable<ManVo>>(await response.Content.ReadAsStringAsync()).Cast<ManVo>().ToList();
