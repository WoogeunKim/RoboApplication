...
{
    //설비 관리
    public sealed class M6511ViewModel : ViewModelBase, INotifyPropertyChanged
    {

        private string _title = "조립 작업지시등록";
        ...
        ////
        private M6511MasterDialog masterDialog;


        public M6511ViewModel() 
        {
            SYSTEM_CODE_VO();
            //Refresh();
        }
       ...
       public async void Refresh()
        {
            try
            {
                using (HttpResponseMessage response = await SystemProperties.PROGRAM_HTTP.PostAsync("m6511/mst", new StringContent(JsonConvert.SerializeObject(new ManVo() { CHNL_CD = SystemProperties.USER_VO.CHNL_CD, ROUT_CD = "SA", A_ROUT_CD = new string[] { "SA", "TA" } }), System.Text.Encoding.UTF8, "application/json")))
                {
        ...
        async void OnSelectedMasterItemChanged(string _LOT_DIV_NO = null)
        {
            try
            {
                ...
                using (HttpResponseMessage response = await SystemProperties.PROGRAM_HTTP.PostAsync("m6511/dtl", new StringContent(JsonConvert.SerializeObject(SelectedMasterItem), System.Text.Encoding.UTF8, "application/json")))
                {
                    if (response.IsSuccessStatusCode)
                    {
        ...
        {
            if (this._selectMasterItem == null) { return; }
            masterDialog = new M6511MasterDialog(SelectedDetailItem);
            ...
            try
            {
                ...
                if (delDao != null)
                {
                    ...
                    if (result == MessageBoxResult.Yes)
                    {
                        ...
                        using (HttpResponseMessage response = await SystemProperties.PROGRAM_HTTP.PostAsync("m6511/dtl/d", new StringContent(JsonConvert.SerializeObject(delDao), System.Text.Encoding.UTF8, "application/json")))
                        {
                            if (response.IsSuccessStatusCode)
                            {
                                int _Num = 0;
        ...
        public async void ReportContact()
        {
            try
            {
                ...
                MessageBoxResult result = WinUIMessageBox.Show("[" + this.SelectedDetailItem.LOT_DIV_NO + " / " + this.SelectedDetailItem.MAKE_ST_DT + "] 작지 발행 하시겠습니까?", _title, MessageBoxButton.YesNo, MessageBoxImage.Question);
                if (result == MessageBoxResult.Yes)
                {
                    ...
                    using (HttpResponseMessage response = await SystemProperties.PROGRAM_HTTP.PostAsync("m6500/report", new StringContent(JsonConvert.SerializeObject(SelectedDetailItem), System.Text.Encoding.UTF8, "application/json")))
                    {
                        if (response.IsSuccessStatusCode)
                        {
                            ...
                            if (_reportList.Count > 0)
                            {
                                ...
                                _reportList[0].FM_DT = this.SelectedDetailItem.PROD_PLN_DT;
                                _reportList[0].TO_DT = this.SelectedDetailItem.PROD_PLN_DT;
