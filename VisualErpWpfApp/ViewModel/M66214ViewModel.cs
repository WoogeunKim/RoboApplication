...
{
    //표준 공정 관리
    public sealed class M66214ViewModel : ViewModelBase, INotifyPropertyChanged
    {
       ...
       public async void Refresh( string _ITM_CD = null )
            ...
            else if(_obj.Equals("B"))
            {
                ...
                using (HttpResponseMessage responseX = await SystemProperties.PROGRAM_HTTP.PostAsync("m66213/tree/mst", new StringContent(JsonConvert.SerializeObject(SelectedDetailItem), System.Text.Encoding.UTF8, "application/json")))
                {
                    if (responseX.IsSuccessStatusCode)
                    {
                        ...
                        //
                        _manVo.CHNL_CD = SelectedMasterItem.CHNL_CD;
        ...
        public async void EditMasterContact()
        {
            ...
            using (HttpResponseMessage responseX = await SystemProperties.PROGRAM_HTTP.PostAsync("m66213/tree/mst", new StringContent(JsonConvert.SerializeObject(new ManVo() { CHNL_CD = SystemProperties.USER_VO.CHNL_CD, CMPO_CD = SelectedTreeItem.GBN }), System.Text.Encoding.UTF8, "application/json")))
            {
                if (responseX.IsSuccessStatusCode)
                {
                    ManVo _manVo = JsonConvert.DeserializeObject<ManVo>(await responseX.Content.ReadAsStringAsync());
        ...
        public async void DelMasterContact()
        {
            try
            {
                ...
                if (delDao != null)
                {
                    ...
                    {
                        using (HttpResponseMessage response = await SystemProperties.PROGRAM_HTTP.PostAsync("m66213/mst/d", new StringContent(JsonConvert.SerializeObject(new ManVo() { ROUT_ITM_CD = SelectedMasterItem.ITM_CD, CMPO_CD = delDao.CMPO_CD, CHNL_CD = SystemProperties.USER_VO.CHNL_CD  }), System.Text.Encoding.UTF8, "application/json")))
                        {
        ...
        public async void CopyItem()
        {
            try
            {
                ...
                if (isDialog)
                {
                    Refresh(copyDialog.updateDao.TO_DT);
                }
            }
            catch (System.Exception eLog)
            {
