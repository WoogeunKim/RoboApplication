...
{
    public partial class P4401DetailDialog : DXWindow
    {
        ...
        public P4401DetailDialog(PurVo Dao)
        {
            InitializeComponent();
            ...
            if (Dao.MN_CO_FLG.ToString().Equals("1"))
            {
            ...
            //
            //
            if (Dao.CRNT_PRC_FLG.ToString().Equals("1"))
            {
        ...
        public async void SYSTEM_CODE_VO()
        {
            ...
            using (HttpResponseMessage response = await SystemProperties.PROGRAM_HTTP.GetAsync("s131/dtl/" + Properties.Settings.Default.SettingChnl + "/" + "S-007"))
            {
                if (response.IsSuccessStatusCode)
                {
                    ...
                    if (!string.IsNullOrEmpty(this.orgDao.CURR_CD))
                    {
                        this.combo_CURR_CD.SelectedItem = (this.combo_CURR_CD.ItemsSource as List<SystemCodeVo>).Where(x => x.CLSS_CD.Equals(this.orgDao.CURR_CD)).FirstOrDefault<SystemCodeVo>();
                    }
                }
            }
            ...
            using (HttpResponseMessage response = await SystemProperties.PROGRAM_HTTP.PostAsync("s143", new StringContent(JsonConvert.SerializeObject(new SystemCodeVo() { DELT_FLG = "N", CO_TP_CD = null, AREA_CD = SystemProperties.USER_VO.EMPE_PLC_NM, CHNL_CD = SystemProperties.USER_VO.CHNL_CD }), System.Text.Encoding.UTF8, "application/json")))
            {
                if (response.IsSuccessStatusCode)
                {
                    ...
                    if (!string.IsNullOrEmpty(this.orgDao.CO_NO))
                    {
                        this.combo_CO_NO.SelectedItem = (this.combo_CO_NO.ItemsSource as List<SystemCodeVo>).Where(x => x.CO_NO.Equals(this.orgDao.CO_NO)).FirstOrDefault<SystemCodeVo>();
                    }
                }
            }
        }
