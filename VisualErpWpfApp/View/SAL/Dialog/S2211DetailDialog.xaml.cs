...
{
    public partial class S2211DetailDialog : DXWindow
    {
        ...
        public S2211DetailDialog(SaleVo Dao)
        {
            InitializeComponent();
        ...
        private async void ViewGridRight1_Refresh()
        {
            try
            {
                using (HttpResponseMessage response = await SystemProperties.PROGRAM_HTTP.PostAsync("s2211/popupitems", new StringContent(JsonConvert.SerializeObject(new SaleVo() { CHNL_CD = SystemProperties.USER_VO.CHNL_CD }), System.Text.Encoding.UTF8, "application/json")))
                {
        ...
        async void ViewGridLeft_SelectedItemChanged(object sender, DevExpress.Xpf.Grid.SelectedItemChangedEventArgs e)
        {
            try
            {
                ...
                {
                    using (HttpResponseMessage response = await SystemProperties.PROGRAM_HTTP.PostAsync("s2211/popupitems", new StringContent(JsonConvert.SerializeObject(new SaleVo()
                    {
        ...
        async void combo_ITM_GRP_CLSS_NM_SelectedIndexChanged(object sender, RoutedEventArgs e)
        {
            if (!string.IsNullOrEmpty(this.combo_ITM_GRP_CLSS_NM.Text))
            {
                ...
                SystemCodeVo itmGrpClssNmVo = this.combo_ITM_GRP_CLSS_NM.SelectedItem as SystemCodeVo;
                using (HttpResponseMessage response = await SystemProperties.PROGRAM_HTTP.PostAsync("s2211/popupleft", new StringContent(JsonConvert.SerializeObject(new SystemCodeVo() { ITM_GRP_CLSS_CD = itmGrpClssNmVo.CLSS_CD, CHNL_CD = SystemProperties.USER_VO.CHNL_CD }), System.Text.Encoding.UTF8, "application/json")))
                {
                    if (response.IsSuccessStatusCode)
                    {
        ...
        private async void OKButton_Click(object sender, RoutedEventArgs e)
        {
            try
            {
                ...
                //foreach (SaleVo s in tmpDao)
                //{
                //    if (s.SL_ITM_QTY <= 0)
                //    {
                //        WinUIMessageBox.Show("[" + s.SL_ITM_CD + "] " + s.ITM_NM + " 의 수량값이 잘못되었습니다.", "[에러]" + title, MessageBoxButton.OK, MessageBoxImage.Warning);
                //        return;
                //    }
                //}

                if(this.tmpDao.Any<SaleVo>(s => (s.SL_ITM_QTY <= 0)))
                {
                ...
                {
                    //전체 삭제
                    using (HttpResponseMessage response = await SystemProperties.PROGRAM_HTTP.PostAsync("s2211/dtl/d", new StringContent(JsonConvert.SerializeObject(this.orgDao), System.Text.Encoding.UTF8, "application/json")))
                    {
                        if (response.IsSuccessStatusCode)
                        {
                ...
                //{
                using (HttpResponseMessage response = await SystemProperties.PROGRAM_HTTP.PostAsync("s2211/dtl/i", new StringContent(JsonConvert.SerializeObject(tmpDao), System.Text.Encoding.UTF8, "application/json")))
                {
                    if (response.IsSuccessStatusCode)
                    {
                        string result = await response.Content.ReadAsStringAsync();
...
//}
        ...
        public async void SYSTEM_CODE_VO()
        {
            ...
            using (HttpResponseMessage response = await SystemProperties.PROGRAM_HTTP.PostAsync("s2211/popup", new StringContent(JsonConvert.SerializeObject(this.orgDao), System.Text.Encoding.UTF8, "application/json")))
            {
                if (response.IsSuccessStatusCode)
                {
