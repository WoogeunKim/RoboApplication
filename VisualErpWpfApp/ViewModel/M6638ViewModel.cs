...
{
    public sealed class M6638ViewModel : ViewModelBase, INotifyPropertyChanged 
    {
        ...
        private IList<ManVo> selectMstList = new List<ManVo>();   
        //private IList<SaleVo> selectDtlList = new List<SaleVo>();
        private IList<ManVo> selectItemsList = new List<ManVo>();
        private IList<ManVo> selectdItemsList = new List<ManVo>();

        private ManVo selectMstGr = new ManVo();
        ...
        public async void Refresh(string _RLSE_CMD_NO = null)
        {
            try
            {
                ...
                using (HttpResponseMessage response = await SystemProperties.PROGRAM_HTTP.PostAsync("m6638/mst", new StringContent(JsonConvert.SerializeObject(_param), System.Text.Encoding.UTF8, "application/json")))
                {
                    if (response.IsSuccessStatusCode)
                    {
                        this.ItemsSelectdList = new List<ManVo>();
                        this.GrItemsSelectList = new List<ManVo>();

                        this.GrMstSelectList = JsonConvert.DeserializeObject<IEnumerable<ManVo>>(await response.Content.ReadAsStringAsync()).Cast<ManVo>().ToList();
                        if (this.GrMstSelectList.Count >= 1)
                        {
                            ...
                            {
                                this.selectMstGr = this.GrMstSelectList.Where(x => x.RLSE_CMD_NO.Equals(_RLSE_CMD_NO)).LastOrDefault<ManVo>();
                            }
                        }
                        else
                        {
        ...
        #region MyRegion
        public IList<ManVo> GrMstSelectList
        {
        ...
        public IList<ManVo> GrItemsSelectList
        {
        ...
        //SelectdItems
        public IList<ManVo> ItemsSelectdList
        {
