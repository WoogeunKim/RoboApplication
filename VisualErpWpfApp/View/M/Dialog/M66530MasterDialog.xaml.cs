...
{
    public partial class M66530MasterDialog : DXWindow
    {

        private string _title = "작업지시서 발행(포장)";
        ...
        public M66530MasterDialog(ManVo Dao)
        {
            InitializeComponent();
            ...
            //this.text_LOT_DIV_RMK.Text = string.Empty;
        ...
        private async void OKButton_Click(object sender, RoutedEventArgs e)
        {
            if (ValueCheckd())
            {
                ...
                if (isEdit == false)
                {
                    ...
                    if (WinUIMessageBox.Show("(포장) 작업 지시서 생성 하시겠습니까?", _title, MessageBoxButton.YesNo, MessageBoxImage.Question) == MessageBoxResult.Yes)
                    {
                        using (HttpResponseMessage response = await SystemProperties.PROGRAM_HTTP.PostAsync("m66520/mst/m/p", new StringContent(JsonConvert.SerializeObject(this.ViewGridDtl.SelectedItems), System.Text.Encoding.UTF8, "application/json")))
                        {
                            if (response.IsSuccessStatusCode)
                            {
        ...
        public Boolean ValueCheckd()
        {
            ...
            //
            //if (Convert.ToInt32(this.text_PROD_PLN_QTY.Text) > Convert.ToInt32(this.orgDao.PROD_PLN_QTY))
            //{
        ...
        private async void PopupRoutQty()
        {
            ...
            using (HttpResponseMessage response = await SystemProperties.PROGRAM_HTTP.PostAsync("m66520/popup/routqty", new StringContent(JsonConvert.SerializeObject(this.orgDao), System.Text.Encoding.UTF8, "application/json")))
            {
                if (response.IsSuccessStatusCode)
                {
                    ...
                    // 착인 표시
                    if (_routQtyList.Any<ManVo>(x => x.ROUT_NM.Equals("착인")) == true)
                    {
                        this.stackpanel_Rout1.Visibility = Visibility.Visible;
                        this.text_A.Text = Convert.ToDecimal(_routQtyList.Where<ManVo>(x => x.ROUT_NM.Equals("착인")).FirstOrDefault<ManVo>()?.LOT_DIV_QTY).ToString("N0");
                    ...
                    // 선공정 표시
                    if (_routQtyList.Any<ManVo>(x => x.ROUT_NM.Equals("선공정")) == true)  // RoutQTYList에 선공정이 하나라도 있는지 확인 후에 T/F 반환(Any)
                    {
                        this.stackpanel_Rout2.Visibility = Visibility.Visible;
                        this.text_B.Text = Convert.ToDecimal(_routQtyList.Where<ManVo>(x => x.ROUT_NM.Equals("선공정")).FirstOrDefault<ManVo>()?.LOT_DIV_QTY).ToString("#,###,##0");
                    }
                    else
                    {
                    ...
                    // 포장 표시
                    if (_routQtyList.Any<ManVo>(x => x.ROUT_NM.Equals("포장")) == true)
                    {
                        this.stackpanel_Rout3.Visibility = Visibility.Visible;
                        this.text_C.Text = Convert.ToDecimal(_routQtyList.Where<ManVo>(x => x.ROUT_NM.Equals("포장")).FirstOrDefault<ManVo>()?.LOT_DIV_QTY).ToString("#,###,##0");
                    }
                    else
                    {
        ...
        public async void SYSTEM_CODE_VO()
        {
            ...
            //구분
            this.lue_DY_NGT_NM.ItemsSource = new List<SystemCodeVo>() { new SystemCodeVo() { CLSS_CD = "E", CLSS_DESC = "내부1" }, new SystemCodeVo() { CLSS_CD = "F", CLSS_DESC = "내부2" }};
