...
using AquilaErpWpfApp3.SAL.View.Dialog;
using AquilaErpWpfApp3.Util;

namespace AquilaErpWpfApp3.ViewModel
{
    public sealed class S2220ViewModel : ViewModelBase, INotifyPropertyChanged
    {
        ...
        private S2220MasterDialog masterDialog;
        ...
        [Command]
        public async void Refresh(string _SL_PLN_NO = null)
        {
            try
            {
                using (HttpResponseMessage response = await SystemProperties.PROGRAM_HTTP.PostAsync("s2220", new StringContent(JsonConvert.SerializeObject(new SaleVo() { SL_PLN_YRMON = Convert.ToDateTime(StartDt).ToString("yyyyMM"), CHNL_CD = SystemProperties.USER_VO.CHNL_CD }), System.Text.Encoding.UTF8, "application/json")))
                {
                    ...
                    if (SelectMstList.Count >= 1)
                    {
                        ...
                        if (string.IsNullOrEmpty(_SL_PLN_NO))
                        {
                            SelectedMstItem = SelectMstList[0];
                        }
                        else
                        {
                            SelectedMstItem = SelectMstList.Where(x => x.SL_PLN_NO.Equals(_SL_PLN_NO)).LastOrDefault<SaleVo>();
                        }
                    }
                    else
                    {
        ...
        {
            masterDialog = new S2220MasterDialog(new SaleVo() { SL_PLN_YRMON = StartDt.ToString("yyyy-MM"), SL_PLN_NM = "정상발주", SL_PLN_QTY = 0 });
            ...
            if (isDialog)
            {
                Refresh(masterDialog.ResultDao.SL_PLN_NO);
            }
        }
        ...
        public void EditDtlContact()
        {
            ...
            masterDialog = new S2220MasterDialog(SelectedMstItem);
            masterDialog.Title = title + " - 수정(" + SelectedMstItem.SL_PLN_NO + ")";
            masterDialog.Owner = Application.Current.MainWindow;
            masterDialog.BorderEffect = BorderEffect.Default;
            masterDialog.BorderEffectActiveColor = new SolidColorBrush(Color.FromRgb(255, 128, 0));
            masterDialog.BorderEffectInactiveColor = new SolidColorBrush(Color.FromRgb(255, 170, 170));
        ...
        public async void DelContact()
        {
            try
            {
                //SaleVo delDao = this._selectMasterItem;
                if (SelectedMstItem != null)
                {
                    MessageBoxResult result = WinUIMessageBox.Show(SelectedMstItem.SL_PLN_NO + " 정말로 삭제 하시겠습니까?", title, MessageBoxButton.YesNo, MessageBoxImage.Question);
                    if (result == MessageBoxResult.Yes)
                    {
                        using (HttpResponseMessage response = await SystemProperties.PROGRAM_HTTP.PostAsync("s2220/d", new StringContent(JsonConvert.SerializeObject(SelectedMstItem), System.Text.Encoding.UTF8, "application/json")))
                        {
                            if (response.IsSuccessStatusCode)
                            {
                                int _Num = 0;
