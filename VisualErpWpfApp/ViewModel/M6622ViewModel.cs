...
{
    //설비 관리
    public sealed class M6622ViewModel : ViewModelBase, INotifyPropertyChanged
    {
        ...
        //
        private M6622MasterDialog masterDialog;


        public M6622ViewModel() 
        {
            Refresh();
        }
        ...
        {
            masterDialog = new M6622MasterDialog(new ManVo());
        ...
        public void EditMasterContact()
        {
            ...
            if (editDao != null)
            {
                masterDialog = new M6622MasterDialog(editDao);
        ...
        public async void DelMasterContact()
        {
            try
            {
                ...
                if (delDao != null)
                {
                    ...
                    {
                        using (HttpResponseMessage response = await SystemProperties.PROGRAM_HTTP.PostAsync("m6622/d", new StringContent(JsonConvert.SerializeObject(delDao), System.Text.Encoding.UTF8, "application/json")))
                        {
                            if (response.IsSuccessStatusCode)
                            {
                                int _Num = 0;
