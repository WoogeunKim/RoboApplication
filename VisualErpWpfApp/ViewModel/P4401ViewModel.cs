...
{
    public sealed class P4401ViewModel : ViewModelBase, INotifyPropertyChanged
    {
        ...
        private P4401DetailDialog masterDialog;
        ...
        public async void Refresh()
        {
            ...
            catch (System.Exception eLog)
            {
                ...
                WinUIMessageBox.Show(eLog.Message, "[" + SystemProperties.PROGRAM_TITLE + "]매입 단가 관리", MessageBoxButton.OK, MessageBoxImage.Error, MessageBoxResult.None, MessageBoxOptions.None);
                return;
            }
        }
        ...
        public void NewDtlContact()
        {
            ...
            masterDialog = new P4401DetailDialog(new PurVo() { ITM_CD = SelectedMstItem.ITM_CD, CNG_APLY_DT = System.DateTime.Now.ToString("yyyy-MM-dd"), CO_UT_PRC = 0, CURR_CD = "KRW", MN_CO_FLG = 0, CRNT_PRC_FLG = 1, CO_ITM_CD = SelectedMstItem.ITM_CD, CO_ITM_NM = SelectedMstItem.ITM_NM });
            masterDialog.Title = "매입 단가 관리 - 추가[" + SelectedMstItem.ITM_CD + "]";
        ...
        public void EditDtlContact()
        {
            ...
            masterDialog = new P4401DetailDialog(SearchDetail);
            masterDialog.Title = "매입 단가 관리 - 수정[" + SelectedMstItem.ITM_CD + "]";
            masterDialog.Owner = Application.Current.MainWindow;
            masterDialog.BorderEffect = BorderEffect.Default;
            masterDialog.BorderEffectActiveColor = new SolidColorBrush(Color.FromRgb(255, 128, 0));
            masterDialog.BorderEffectInactiveColor = new SolidColorBrush(Color.FromRgb(255, 170, 170));
        ...
        public async void DelDtlContact()
        {
            ...
            if (delDao != null)
            {
                    MessageBoxResult result = WinUIMessageBox.Show("[" + delDao.CO_NO + "/" + delDao.CO_NM + "]" + " 정말로 삭제 하시겠습니까?", "매입 단가 관리", MessageBoxButton.YesNo, MessageBoxImage.Question);
                    ...
                    using (HttpResponseMessage response = await SystemProperties.PROGRAM_HTTP.PostAsync("p4401/dtl/d", new StringContent(JsonConvert.SerializeObject(SearchDetail), System.Text.Encoding.UTF8, "application/json")))
                    {
                        if (response.IsSuccessStatusCode)
                        {
                            ...
                            {
                                //실패
                                WinUIMessageBox.Show(resultMsg, "매입 단가 관리", MessageBoxButton.OK, MessageBoxImage.Error);
                                return;
                            }
                            Refresh();

                            //성공
                            WinUIMessageBox.Show("삭제가 완료되었습니다.", "매입 단가 관리", MessageBoxButton.OK, MessageBoxImage.Information, MessageBoxResult.None, MessageBoxOptions.None);
                        }
                    }
            ...
            using (HttpResponseMessage response = await SystemProperties.PROGRAM_HTTP.GetAsync("s131/dtl/" + Properties.Settings.Default.SettingChnl + "/" + "L-001"))
            {
                if (response.IsSuccessStatusCode)
                {
                    ...
                    if (ItemGrpClssCdList.Count > 0)
                    {
                        ...
                        M_ITM_GRP_CLSS_DESC = ItemGrpClssCdList[0];
                    }
                }
            }
