...
{
    //설비 관리
    public sealed class M66550ViewModel : ViewModelBase, INotifyPropertyChanged
    {
        ...
        ////
        private M66550MasterDialog masterDialog;


        public M66550ViewModel() 
        {
            SYSTEM_CODE_VO();
            //Refresh();
        }
       ...
       public async void Refresh()
        ...
        {
            masterDialog = new M66550MasterDialog(new ManVo() { GBN = M_PLN_YRMON?.GBN, WK = M_PLN_YRMON?.WK, WKY_YRMON = M_PLN_YRMON?.WKY_YRMON, PROD_LOC_CD = M_PROD_LOC_NM?.CLSS_CD });
        ...
        public async void ReportContact()
        {
            try
            {
                ...
                if (result == MessageBoxResult.Yes)
                {
                    ...
                    if (this.SelectedMasterItem.CMPO_CD.Contains("TA"))
                    {
                        _param.PCK_FLG = "TA";
                    }
                    else if (this.SelectedMasterItem.CMPO_CD.Contains("SA"))
                    {
                        _param.PCK_FLG = "SA";
                    }
                    _param.A_ROUT_CD = new string[] { "TA", "SA" };
