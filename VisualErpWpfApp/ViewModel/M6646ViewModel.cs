...
{
    //표준 공정 관리
    public sealed class M6646ViewModel : ViewModelBase, INotifyPropertyChanged
    {
        ...
        //
        private M6646MasterDialog masterDialog;

       
        public M6646ViewModel() 
        {
            Refresh();
        }
       ...
       public async void Refresh(string _KEY_CD = null)
        {
            try
            {
                using (HttpResponseMessage response = await SystemProperties.PROGRAM_HTTP.PostAsync("m6646", new StringContent(JsonConvert.SerializeObject(new ManVo() { CHNL_CD = SystemProperties.USER_VO.CHNL_CD }), System.Text.Encoding.UTF8, "application/json")))
                {
                    ...
                    if (SelectedMasterViewList.Count > 0)
                    {
                        ...
                        {
                            SelectedMasterItem = SelectedMasterViewList.Where(x => (x.EQ_NO + "_" + x.PLN_MON + "_" + x.PLN_DY +"_" + x.PLN_CD).Equals(_KEY_CD)).LastOrDefault<ManVo>();
                        }
                    }
                    else
                    {
        ...
        {
            masterDialog = new M6646MasterDialog(new ManVo());
            ...
            if (isDialog)
            {
                Refresh((masterDialog.resultDomain.EQ_NO + "_" + masterDialog.resultDomain.TMP_PLN_MON + "_" + masterDialog.resultDomain.TMP_PLN_DY + "_" + masterDialog.resultDomain.PLN_CD));
        ...
        public void EditMasterContact()
        {
            ...
            if (editDao != null)
            {
                masterDialog = new M6646MasterDialog(editDao);
        ...
        public async void DelMasterContact()
        {
            try
            {
                ...
                if (delDao != null)
                {
                    MessageBoxResult result = WinUIMessageBox.Show(delDao.EQ_NM + " / " + delDao.PLN_MON + " / " + delDao.PLN_DY + " 정말로 삭제 하시겠습니까?", _title, MessageBoxButton.YesNo, MessageBoxImage.Question);
                    if (result == MessageBoxResult.Yes)
                    {
                        using (HttpResponseMessage response = await SystemProperties.PROGRAM_HTTP.PostAsync("m6646/d", new StringContent(JsonConvert.SerializeObject(delDao), System.Text.Encoding.UTF8, "application/json")))
                        {
                            if (response.IsSuccessStatusCode)
                            {
                                int _Num = 0;
