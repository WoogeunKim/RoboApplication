...
{
    //표준 공정 관리
    public sealed class M6629ViewModel : ViewModelBase, INotifyPropertyChanged
    {
        ...
        //
        private M6629MasterDialog masterDialog;


        public M6629ViewModel() 
        {
            Refresh();
        }

        [Command]
       public async void Refresh(string _ROUT_TP_CD = null)
        {
            try
            {
                //제품 목록
                using (HttpResponseMessage responseX = await SystemProperties.PROGRAM_HTTP.PostAsync("m6629/mst", new StringContent(JsonConvert.SerializeObject(new ManVo() { CHNL_CD = SystemProperties.USER_VO.CHNL_CD, CRE_USR_ID = SystemProperties.USER, UPD_USR_ID = SystemProperties.USER }), System.Text.Encoding.UTF8, "application/json")))
                {
                    ...
                    if (SelectedMasterViewList.Count > 0)
                    {
                        ...
                        if (string.IsNullOrEmpty(_ROUT_TP_CD))
                        {
                            SelectedMasterItem = SelectedMasterViewList[0];
                        }
                        else
                        {
                            SelectedMasterItem = SelectedMasterViewList.Where(x => x.ROUT_TP_CD.Equals(_ROUT_TP_CD)).LastOrDefault<ManVo>();
                        }
                    }
                    else
                    {
        ...
        async void OnSelectedMasterItemChanged()
        {
            try
            {
                ...
                //공정 유형
                using (HttpResponseMessage responseX = await SystemProperties.PROGRAM_HTTP.PostAsync("m6629/popup", new StringContent(JsonConvert.SerializeObject(SelectedMasterItem), System.Text.Encoding.UTF8, "application/json")))
                {
                    if (responseX.IsSuccessStatusCode)
                    {
                ...
                using (HttpResponseMessage responseX = await SystemProperties.PROGRAM_HTTP.PostAsync("m6629/dtl", new StringContent(JsonConvert.SerializeObject(SelectedMasterItem), System.Text.Encoding.UTF8, "application/json")))
                {
                    if (responseX.IsSuccessStatusCode)
                    {
        ...
        {
            masterDialog = new M6629MasterDialog(new ManVo());
            ...
            if (isDialog)
            {
                Refresh(masterDialog.resultDomain.ROUT_TP_CD);
            }
        }
        ...
        {
            if (SelectedMasterItem == null) { return; }
            masterDialog = new M6629MasterDialog(SelectedMasterItem);
        ...
        public async void DelMasterContact()
        {
            try
            {
                ...
                if (delDao != null)
                {
                    MessageBoxResult result = WinUIMessageBox.Show(delDao.ROUT_TP_CD + "/" + delDao.ROUT_TP_NM + " 정말로 삭제 하시겠습니까?", _title, MessageBoxButton.YesNo, MessageBoxImage.Question);
                    if (result == MessageBoxResult.Yes)
                    {
                        using (HttpResponseMessage response = await SystemProperties.PROGRAM_HTTP.PostAsync("m6629/mst/d", new StringContent(JsonConvert.SerializeObject(delDao), System.Text.Encoding.UTF8, "application/json")))
                        {
        ...
        //}
        [Command]
        public async void NewDetailContact()
        {
            if (SelectedPopupItem == null) { return; }

            int _Num = 0;
            using (HttpResponseMessage response = await SystemProperties.PROGRAM_HTTP.PostAsync("m6629/dtl/i", new StringContent(JsonConvert.SerializeObject(SelectedPopupItem), System.Text.Encoding.UTF8, "application/json")))
            {
                if (response.IsSuccessStatusCode)
                {
        ...
        public async void DelDetailContact()
        {
            try
            {
                ...
                //    if (result == MessageBoxResult.Yes)
                //    {
                using (HttpResponseMessage response = await SystemProperties.PROGRAM_HTTP.PostAsync("m6629/dtl/d", new StringContent(JsonConvert.SerializeObject(SelectedDetailItem), System.Text.Encoding.UTF8, "application/json")))
                {
                    if (response.IsSuccessStatusCode)
                    {
                        int _Num = 0;
