...
{
    public partial class P441106MasterDialog : DXWindow
    {
        ...
        public P441106MasterDialog(PurVo Dao)
        {
            InitializeComponent();
            ...
            {
                //PUR_ORD_NO = Dao.PUR_ORD_NO,
                PUR_NO = Dao.PUR_NO,

                PUR_DT = Dao.PUR_DT,

               // PUR_DUE_DT = Dao.PUR_DUE_DT,
                DUE_DT = Dao.DUE_DT,

                PUR_CO_CD = Dao.PUR_CO_CD,
                CO_NO = Dao.CO_NO,
                CO_NM = Dao.CO_NM,
                PUR_ITM_CD = Dao.PUR_ITM_CD,
                PUR_ITM_NM = Dao.PUR_ITM_NM,
                PUR_EMPE_ID = Dao.PUR_EMPE_ID,

               // PUR_CLZ_FLG = Dao.PUR_CLZ_FLG,
                DELT_FLG = Dao.DELT_FLG,

                PUR_WK_CD = Dao.PUR_WK_CD,
                PUR_RMK = Dao.PUR_RMK,
                USR_NM = Dao.USR_NM,
                QC_CD = Dao.QC_CD,
                PUR_AMT = Dao.PUR_AMT,
            ...
            if (Dao.PUR_NO != null)
            {
                this.text_PUR_NO.IsReadOnly = true;
                this.isEdit = true;
                //
                ////마감 처리 후 수정 불가능
        ...
        private async void OKButton_Click(object sender, RoutedEventArgs e)
        {
            if (ValueCheckd())
            {
                ...
                if (isEdit == false)
                {
                    ...
                    using (HttpResponseMessage response = await SystemProperties.PROGRAM_HTTP.PostAsync("p441106/mst/i", new StringContent(JsonConvert.SerializeObject(this.updateDao), System.Text.Encoding.UTF8, "application/json")))
                    {
                ...
                else
                {
                    ...
                    using (HttpResponseMessage response = await SystemProperties.PROGRAM_HTTP.PostAsync("p441106/mst/u", new StringContent(JsonConvert.SerializeObject(this.updateDao), System.Text.Encoding.UTF8, "application/json")))
                    {
                        if (response.IsSuccessStatusCode)
                        {
                            string result = await response.Content.ReadAsStringAsync();
        ...
        public Boolean ValueCheckd()
        {
            ...
            {
                WinUIMessageBox.Show("[발주 일자] 입력 값이 맞지 않습니다.", _title  , MessageBoxButton.OK, MessageBoxImage.Warning);
                this.text_PUR_DT.IsTabStop = true;
                this.text_PUR_DT.Focus();
                return false;
            }
            ...
            {
                WinUIMessageBox.Show("[매입처] 입력 값이 맞지 않습니다.", _title, MessageBoxButton.OK, MessageBoxImage.Warning);
        ...
        private PurVo getDomain()
        {
            ...
            Dao.PUR_NO = this.text_PUR_NO.Text;
        ...
        public async void SYSTEM_CODE_VO()
        {
            ...
            using (HttpResponseMessage response = await SystemProperties.PROGRAM_HTTP.PostAsync("s143", new StringContent(JsonConvert.SerializeObject(new SystemCodeVo() { DELT_FLG = "N", SEEK_AP = "AR", SEEK = "AR", CO_TP_CD = "AR", AREA_CD = SystemProperties.USER_VO.EMPE_PLC_NM, CHNL_CD = SystemProperties.USER_VO.CHNL_CD }), System.Text.Encoding.UTF8, "application/json")))
            {
                if (response.IsSuccessStatusCode)
                {
