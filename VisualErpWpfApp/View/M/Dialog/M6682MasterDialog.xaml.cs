...
namespace AquilaErpWpfApp3.View.M.Dialog
{
    ...
    /// </summary>
    public partial class M6682MasterDialog : DXWindow
    {
        ...
        private string _title = "금형도면이력관리";
        ...
        public string _MOLD_CD;
        //private int? _ITM_DWG_SEQ;

        public M6682MasterDialog(ManVo Dao)
        {
            InitializeComponent();
            ...
            ManVo copyDao = new ManVo()
            {
                ...
                MOLD_NM = Dao.MOLD_NM,
                MOLD_NO = Dao.MOLD_NO,
                ITM_MOLD_SEQ = Dao.ITM_MOLD_SEQ,
                MOLD_FILE = Dao.MOLD_FILE,
                MOLD_FILE_NM = Dao.MOLD_FILE_NM,
        ...
        void btn_file_Click(object sender, RoutedEventArgs e)
        {
            ...
            if (result == true)
            {
                ...
                {
                    WinUIMessageBox.Show("[파일 크기] 3MB를 초과 하였습니다", "[유효검사]금형도면이력관리", MessageBoxButton.OK, MessageBoxImage.Warning);
                    this.text_EQ_FILE_NM.Text = "";
                    this.fileByte = new byte[0];
                    return;
                }
        ...
        private async void OKButton_Click(object sender, RoutedEventArgs e)
        {
            if (ValueCheckd())
            {
                ...
                if (isEdit == false)
                {
                    ...
                    using (HttpResponseMessage response = await SystemProperties.PROGRAM_HTTP.PostAsync("m6682/i", new StringContent(JsonConvert.SerializeObject(this.updateDao), System.Text.Encoding.UTF8, "application/json")))
                    {
        ...
        public Boolean ValueCheckd()
        {
            ...
            {
                WinUIMessageBox.Show("[금형도면명] 입력 값이 맞지 않습니다.", "[유효검사]금형도면이력관리", MessageBoxButton.OK, MessageBoxImage.Warning);
        ...
        private ManVo getDomain()
        {
            ...
            //Dao.MOLD_CD = this._MOLD_CD;
            Dao.MOLD_NO = this._MOLD_CD;
            //Dao.ITM_DWG_SEQ = this._ITM_DWG_SEQ;

            Dao.MOLD_NM = this.text_EQ_NM.Text;
            Dao.MOLD_FILE_NM = this.text_EQ_FILE_NM.Text;
            Dao.MOLD_RMK = this.text_EQ_RMK.Text;
            Dao.DELT_FLG = "N";
            ...
            //
            //Dao.EQ_FILE_NM = this.text_EQ_FILE_NM.Text;
            if (fileByte == null)
            {
                Dao.MOLD_FILE = new byte[0];
            }
            else
            {
                Dao.MOLD_FILE = this.fileByte;
            }
