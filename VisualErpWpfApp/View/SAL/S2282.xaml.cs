...
{
    public partial class S2282 : UserControl
    {
        ...
        public S2282()
        {
            try
            {
                DataContext = new S2282ViewModel();

                InitializeComponent();

                this.pivotGrid.CellDoubleClick += new DevExpress.Xpf.PivotGrid.PivotCellEventHandler(pivotGrid_CellDoubleClick);

                this.chartControl.Diagram = ChartFactory.GenerateDiagram(typeof(LineSeries2D), false);
                //this.chartControl.Diagram.Series.
            ...
            {
                WinUIMessageBox.Show(eLog.Message, "[에러]매출 요약(기간별)", MessageBoxButton.OK, MessageBoxImage.Error);
                return;
            }
        }
        ...
        void btn_ViewPage_print_Click(object sender, RoutedEventArgs e)
        {
            ...
            using (PrintableControlLink prtLink = new PrintableControlLink(this.pivotGrid))
            {
                ...
                prtLink.DocumentName = "매출 요약(기간별) Print";
                ...
                prtLink.ShowPrintPreviewDialog(Application.Current.MainWindow, "매출 요약(기간별)");
            }
