...
{
    public sealed class T91103ViewModel : ViewModelBase, INotifyPropertyChanged
    {
        ...
        private T91103MasterDialog masterDialog;
       ...
       [Command]
       public async void Refresh(string _BAD_CD = null)
        {
            try
            {

                using (HttpResponseMessage response = await SystemProperties.PROGRAM_HTTP.PostAsync("t91103/", new StringContent(JsonConvert.SerializeObject(new TecVo() {CHNL_CD = SystemProperties.USER_VO.CHNL_CD}), System.Text.Encoding.UTF8, "application/json")))
                {
                    ...
                    if (SelectMstList.Count >= 1)
                    {
                        isM_UPDATE = true;
                        isM_DELETE = true;
                        if (string.IsNullOrEmpty(_BAD_CD))
                        {
                            SelectedMstItem = SelectMstList[0];
                        }
                        else
                        {
                            SelectedMstItem = SelectMstList.Where(x => x.BAD_CD.Equals(_BAD_CD)).LastOrDefault<TecVo>();
                        }
                    }
                    else
                    {
        ...
        public void NewContact()
        {
            ...
            //{
              //  string _coNm = this.SelectedMstItems[0].LOT_NO;
                ...
                masterDialog = new T91103MasterDialog(new TecVo() { });
                ...
                if (isDialog)
                {
                    Refresh(masterDialog.updateDao.BAD_CD);
        ...
        public void EditContact()
        {
            ...
            masterDialog = new T91103MasterDialog(SelectedMstItem);
