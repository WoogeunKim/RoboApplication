...
{
    public sealed class S1161ViewModel : ViewModelBase, INotifyPropertyChanged
    {
        private string title = "근태기준정보관리";

        private S1161MasterDialog s1161MasterDialog;

        //private ICommand _searchDialogCommand;
        //private ICommand _newDialogCommand;
        //private ICommand _editDialogCommand;
        public S1161ViewModel() 
        {
            //사업장
            SYSTEM_CODE_VO();
            // - Refresh();
        ...
        [Command]
        public async void Refresh(string WRK_TP_CD = null)
        {
            try
            {
                using (HttpResponseMessage response = await SystemProperties.PROGRAM_HTTP.PostAsync("s1161", new StringContent(JsonConvert.SerializeObject(new SystemCodeVo() { CHNL_CD = SystemProperties.USER_VO.CHNL_CD }), System.Text.Encoding.UTF8, "application/json")))
                {
                    ...
                    if (SelectMstList.Count >= 1)
                    {
                        ...
                        if (string.IsNullOrEmpty(WRK_TP_CD))
                        {
                            SelectedMstItem = SelectMstList[0];
                        }
                        else
                        {
                            SelectedMstItem = SelectMstList.Where(x => (x.WRK_TP_CD).Equals(WRK_TP_CD)).LastOrDefault<SystemCodeVo>();
                        }
                    }
                    else
                    {
        ...
        public void NewContact()
        {
            ...
            try
            {
                ...
                s1161MasterDialog = new S1161MasterDialog(new SystemCodeVo() {  });
                s1161MasterDialog.Title = this.title + " - 추가";
