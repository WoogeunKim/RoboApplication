...
{
    public sealed class M66530ViewModel : ViewModelBase, INotifyPropertyChanged
    {
        ...
        private string _title = "작업지지서 발행(포장)";

        private M66530MasterDialog masterDialog;

        private M66530PrintDialog printDialog;

        //private ICommand _searchDialogCommand;

        public M66530ViewModel() 
        {
            StartDt = System.DateTime.Now;
            EndDt = System.DateTime.Now;

            //PlnDt = System.DateTime.Now;
            Start_Prod_Dt = System.DateTime.Now;
            End_Prod_Dt = System.DateTime.Now;
          ...
          [Command]
          public async void Refresh(string _obj)
          {
              ...
              try
              {
                ...
                {
                    _param.FM_DT = (Start_Prod_Dt).ToString("yyyyMMdd");
                    _param.TO_DT = (End_Prod_Dt).ToString("yyyyMMdd");
                }
                ...
                using (HttpResponseMessage response = await SystemProperties.PROGRAM_HTTP.PostAsync("m66520/mst", new StringContent(JsonConvert.SerializeObject(_param), System.Text.Encoding.UTF8, "application/json")))
                    {

                        SelectMstList = JsonConvert.DeserializeObject<IEnumerable<ManVo>>(await response.Content.ReadAsStringAsync()).Cast<ManVo>().ToList();
                        Title = "[기간]" + (StartDt).ToString("yyyy-MM-dd") + "~" + (EndDt).ToString("yyyy-MM-dd") + ",      [사업장]" + M_SL_AREA_NM.CLSS_DESC + ",      [구분]포장"; ;
        ...
        public async void NewContact()
        {
            try
            {
                ...
                masterDialog = new M66530MasterDialog(SelectedMstItem);
        ...
        public async void DelContact()
        {
            try
            {
                ...
                if (SearchDetailJob != null)
                {
                    ...
                    {
                        using (HttpResponseMessage response = await SystemProperties.PROGRAM_HTTP.PostAsync("m66520/mst/d", new StringContent(JsonConvert.SerializeObject(SearchDetailJob), System.Text.Encoding.UTF8, "application/json")))
                        {
                            if (response.IsSuccessStatusCode)
                            {
                                ...
                                using (HttpResponseMessage responseX = await SystemProperties.PROGRAM_HTTP.PostAsync("m66520/dtl/d", new StringContent(JsonConvert.SerializeObject(SearchDetailJob), System.Text.Encoding.UTF8, "application/json")))
                                {
        ...
        [Command]
        public async void PrintContactPak()
        {
            try
            {
                ...
                using (HttpResponseMessage response = await SystemProperties.PROGRAM_HTTP.PostAsync("m66520/report", new StringContent(JsonConvert.SerializeObject(SearchDetailJob), System.Text.Encoding.UTF8, "application/json")))
                {
                    if (response.IsSuccessStatusCode)
                    {
                        ...
                        if (_ItmList.Count > 0)
                        {
                            ...
                            printDialog = new M66530PrintDialog(_ItmList);
                            printDialog.Title = "포장 지시 및 기록서 - " + SelectDtlItmList[0].LOT_DIV_NO;
                            printDialog.Owner = Application.Current.MainWindow;
                            printDialog.BorderEffect = BorderEffect.Default;
                            ////masterDialog.BorderEffectActiveColor = new SolidColorBrush(Color.FromRgb(255, 128, 0));
                            ////masterDialog.BorderEffectInactiveColor = new SolidColorBrush(Color.FromRgb(255, 170, 170));
                            bool isDialog = (bool)printDialog.ShowDialog();
                            if (isDialog)
                            {
                ...
                using (HttpResponseMessage response = await SystemProperties.PROGRAM_HTTP.PostAsync("m66520/report", new StringContent(JsonConvert.SerializeObject(SearchDetailJob), System.Text.Encoding.UTF8, "application/json")))
                {
                    if (response.IsSuccessStatusCode)
                    {
                        ...
                        if (_ItmList.Count > 0)
                        {
                            ...
                            _ItmList[0].ROUT_NM = SearchDetailJob.ROUT_NM;
                            _ItmList[0].DY_NGT_NM = SearchDetailJob.DY_NGT_NM;
                            ...
                            SearchDetailJob.WRK_DIR_PPR_FLG = "Y";
                            SearchDetailJob.LOT_DIV_SEQ = null;
        ...
        DateTime _start_Prod_Dt;
        public DateTime Start_Prod_Dt
        {
            get { return _start_Prod_Dt; }
            set { SetProperty(ref _start_Prod_Dt, value, () => Start_Prod_Dt); }
        }

        DateTime _end_Prod_Dt;
        public DateTime End_Prod_Dt
        {
            get { return _end_Prod_Dt; }
            set { SetProperty(ref _end_Prod_Dt, value, () => End_Prod_Dt); }
        }
        //DateTime _plnDt;
        //public DateTime PlnDt
        //{
        //    get { return _plnDt; }
        //    set { SetProperty(ref _plnDt, value, () => PlnDt); }
        //}
        ...
        private async void SearchDetailItem()
        {
            ...
            SelectedMstItem.PCK_FLG = "P";
            using (HttpResponseMessage response = await SystemProperties.PROGRAM_HTTP.PostAsync("m66520/dtl", new StringContent(JsonConvert.SerializeObject(SelectedMstItem), System.Text.Encoding.UTF8, "application/json")))
            {
                if (response.IsSuccessStatusCode)
                {
            ...
            using (HttpResponseMessage response = await SystemProperties.PROGRAM_HTTP.PostAsync("m66520/dtl", new StringContent(JsonConvert.SerializeObject(SelectedMstItem), System.Text.Encoding.UTF8, "application/json")))
            {
                if (response.IsSuccessStatusCode)
                {
                    this.SelectDtlViewList = JsonConvert.DeserializeObject<IEnumerable<ManVo>>(await response.Content.ReadAsStringAsync()).Cast<ManVo>().ToList();
                }
                //SelectDtlItmList = manClient.M6623SelectDetail(SelectedMenuItem);
                if (SelectDtlViewList.Count > 0)
                {
                    //isM_UPDATE = true;
                    //isM_DELETE = true;

                    SearchDetailView = SelectDtlViewList[0];
                }
                else
                {
        ...
        private IList<ManVo> selectDtlViewList;
        public IList<ManVo> SelectDtlViewList
        {
            get { return selectDtlViewList; }
            set { SetProperty(ref selectDtlViewList, value, () => SelectDtlViewList); }
        }


        ManVo _searchDetailView;
        public ManVo SearchDetailView
        {
            get
            {
            ...
            set
            {
                if (value != null)
                {
                    SetProperty(ref _searchDetailView, value, () => SearchDetailView);
                }
            }
        }
        ...
        private async void SearchItems()
        {
            ...
            using (HttpResponseMessage response = await SystemProperties.PROGRAM_HTTP.PostAsync("m66520/dtl/two", new StringContent(JsonConvert.SerializeObject(SearchDetailJob), System.Text.Encoding.UTF8, "application/json")))
            {
                if (response.IsSuccessStatusCode)
                {
