...
{
    public sealed class S2326ViewModel : ViewModelBase, INotifyPropertyChanged
    {
        private string title = "거래명세서 일괄발행";
        ...
        public async  void Refresh()
        {
            try
            {
                ...
                //사업장
                _param.SL_AREA_CD = M_SL_AREA_NM.CHNL_CD;
        ...
        private async void SelectMstDetail()
        {
            try
            {
                ...
                using (HttpResponseMessage response = await SystemProperties.PROGRAM_HTTP.PostAsync("s2212/dtl", new StringContent(JsonConvert.SerializeObject(new SaleVo() {CHNL_CD = SystemProperties.USER_VO.CHNL_CD, SL_RLSE_NO = SelectedMstItem.SL_RLSE_NO, RLSE_CMD_NO = SelectedMstItem.RLSE_CMD_NO, RLSE_CMD_SEQ = SelectedMstItem.RLSE_CMD_SEQ, SL_CO_CD = SelectedCoCdItem.SL_CO_CD, AREA_CD = SystemProperties.USER_VO.EMPE_PLC_NM, RLSE_PRT_FLG = (M_RLSE_PRT_FLG.Equals("전체") ? null : (M_RLSE_PRT_FLG.Equals("출력") ? "Y" : "N")) }), System.Text.Encoding.UTF8, "application/json")))
                {
                    if (response.IsSuccessStatusCode)
                    {
                        this.SelectCoCdList = JsonConvert.DeserializeObject<IEnumerable<SaleVo>>(await response.Content.ReadAsStringAsync()).Cast<SaleVo>().ToList();
        ...
        public void ReportContact()
        {
            ...
            //ObservableCollection<SpecificationOnTransVo> allItems = new ObservableCollection<SpecificationOnTransVo>(saleOrderClient.S221SelectMstSpecificationOnTransReportList(new JobVo() { RLSE_CMD_NO = SelectedMstItem.RLSE_CMD_NO, RLSE_CMD_SEQ = SelectedMstItem.RLSE_CMD_SEQ, SL_CO_CD = SelectedCoCdItem.SL_CO_CD, CRE_USR_ID = SystemProperties.USER }));
            ...
            ////JobVo resultVo = saleOrderClient.S2213UpdateDtl(new JobVo() { SL_RLSE_NO = SelectedMstItem.SL_RLSE_NO, PRN_FLG = "Y", UPD_USR_ID = SystemProperties.USER });
            ////if (!resultVo.isSuccess)
            ////{
            ////    //실패
            ////    WinUIMessageBox.Show(resultVo.Message, "[에러]" + title, MessageBoxButton.OK, MessageBoxImage.Error);
