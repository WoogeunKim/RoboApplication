...
{
    public sealed class S2229ViewModel : ViewModelBase, INotifyPropertyChanged
    {
          ...
          public async void Refresh()
          {
              ...
              try
              {
                ...
                using (HttpResponseMessage response = await SystemProperties.PROGRAM_HTTP.PostAsync("s2229/mst", new StringContent(JsonConvert.SerializeObject(new SaleVo() { FM_DT = (StartDt).ToString("yyyyMMdd"), TO_DT = (EndDt).ToString("yyyyMMdd"), AREA_CD = M_SL_AREA_NM.CLSS_CD, SL_CO_CD = (M_SL_CO_NM == null ? null : M_SL_CO_NM.CO_NO), CHNL_CD = SystemProperties.USER_VO.CHNL_CD }), System.Text.Encoding.UTF8, "application/json")))
                {
                    ...
                    //SelectMstList = saleOrderClient.S2229SelectMstList(_param);
              ...
              {
                  //DXSplashScreen.Close();
                  WinUIMessageBox.Show(eLog.Message, "[" + SystemProperties.PROGRAM_TITLE + "]매출-일자별원장", MessageBoxButton.OK, MessageBoxImage.Error, MessageBoxResult.None, MessageBoxOptions.None);
                  return;
              }
