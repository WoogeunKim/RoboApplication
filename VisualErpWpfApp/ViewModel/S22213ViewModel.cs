...
{
    public sealed class S22213ViewModel : ViewModelBase, INotifyPropertyChanged
    {
        private string title = "판매출고처리";
        ...
        public async void Refresh()
        {
            try
            {
                ...
                using (HttpResponseMessage response = await SystemProperties.PROGRAM_HTTP.PostAsync("s22213/mst", new StringContent(JsonConvert.SerializeObject(_param), System.Text.Encoding.UTF8, "application/json")))
                {
        ...
        public async void Apply()
        {
            try
            {
                ...
                {
                    MessageBoxResult result = WinUIMessageBox.Show("[ 총 :" + SelectedMstItems.Count + "개 ] 정말로 출고 하시겠습니까?", title, MessageBoxButton.YesNo, MessageBoxImage.Question);
                    if (result == MessageBoxResult.Yes)
                    {
                        using (HttpResponseMessage response = await SystemProperties.PROGRAM_HTTP.PostAsync("s22213/mst/m", new StringContent(JsonConvert.SerializeObject(SelectedMstItems), System.Text.Encoding.UTF8, "application/json")))
                        {
                            int _Num;
                            if (response.IsSuccessStatusCode)
                            {
                                ...
                                {
                                    //실패
                                    WinUIMessageBox.Show(resultMst, title, MessageBoxButton.OK, MessageBoxImage.Error);
                                    return;
                                }

                                WinUIMessageBox.Show("[총 계수 : " + SelectedMstItems.Count + "] - 출고 되었습니다", title, MessageBoxButton.OK, MessageBoxImage.Information);
                            }
                        }
                    }
                }
