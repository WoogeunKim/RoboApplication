...
namespace AquilaErpWpfApp3.View.M.Dialog
{
    ...
    /// </summary>
    public partial class M663104MasterDialog : DXWindow
    {
        ...
        public M663104MasterDialog(ManVo Dao)
        {
            InitializeComponent();
        ...
        private async void OKButton_Click(object sender, RoutedEventArgs e)
        {
            if (ValueCheckd())
            {
                ...
                if (isEdit == false)
                {
                    ...
                    MessageBoxResult result = WinUIMessageBox.Show("[" + this.updateDao.MIX_WEIH_VAL + "Kg]  " + nCnt + "개 " + (nMod == 0 ? "" : "/ 나머지 " + nMod + "Kg") + " 생성 하시겠습니까?", "[추가] 칭량 지시 계획", MessageBoxButton.YesNo, MessageBoxImage.Question);
                    if (result == MessageBoxResult.Yes)
                    {
                        using (HttpResponseMessage response = await SystemProperties.PROGRAM_HTTP.PostAsync("m6631/mst/proc", new StringContent(JsonConvert.SerializeObject(this.updateDao), System.Text.Encoding.UTF8, "application/json")))
                        {
                            if (response.IsSuccessStatusCode)
                            {
                                ...
                                {
                                    //실패
                                    WinUIMessageBox.Show(resultMSG, "[" + SystemProperties.PROGRAM_TITLE + "] 칭량 지시 계획", MessageBoxButton.OK, MessageBoxImage.Error);
                                    return;
                                }

                                //성공
                                WinUIMessageBox.Show("완료 되었습니다", "[추가] 칭량 지시 계획", MessageBoxButton.OK, MessageBoxImage.Information);
                            }
                        }
                ...
                else
                {
                    ...
                    using (HttpResponseMessage response = await SystemProperties.PROGRAM_HTTP.PostAsync("m6631/mst/u", new StringContent(JsonConvert.SerializeObject(this.updateDao), System.Text.Encoding.UTF8, "application/json")))
                    {
                        if (response.IsSuccessStatusCode)
                        {
                            ...
                            {
                                //실패
                                WinUIMessageBox.Show(result, "[" + SystemProperties.PROGRAM_TITLE + "] 칭량 지시 계획", MessageBoxButton.OK, MessageBoxImage.Error);
                                return;
                            }

                            //성공
                            WinUIMessageBox.Show("완료 되었습니다", "[수정] 칭량 지시 계획", MessageBoxButton.OK, MessageBoxImage.Information);
                        }
                    }
        ...
        public Boolean ValueCheckd()
        {
            ...
            {
                WinUIMessageBox.Show("[벌크 코드] 입력 값이 맞지 않습니다.", "[유효검사] 칭량 지시 계획", MessageBoxButton.OK, MessageBoxImage.Warning);
                this.combo_ASSY_ITM_CD.IsTabStop = true;
                this.combo_ASSY_ITM_CD.Focus();
                return false;
            }
            else if (Convert.ToInt32(this.text_TOT_WEIH_VAL.Text) <= 0)
            {
                WinUIMessageBox.Show("[총 중량(Kg)] 입력 값이 맞지 않습니다.", "[유효검사] 칭량 지시 계획", MessageBoxButton.OK, MessageBoxImage.Warning);
            ...
            {
                WinUIMessageBox.Show("[Batch 중량(Kg)] 입력 값이 맞지 않습니다.", "[유효검사] 칭량 지시 계획", MessageBoxButton.OK, MessageBoxImage.Warning);
            ...
            {
                WinUIMessageBox.Show("[작업 일자] 입력 값이 맞지 않습니다.", "[유효검사] 칭량 지시 계획", MessageBoxButton.OK, MessageBoxImage.Warning);
            ...
            {
                WinUIMessageBox.Show("[작업자] 입력 값이 맞지 않습니다.", "[유효검사] 칭량 지시 계획", MessageBoxButton.OK, MessageBoxImage.Warning);
        ...
        public async void SYSTEM_CODE_VO()
        {
            ...
            { 
                ...
                {
                    //수주 번호
                    //FM_DT = System.DateTime.Now.AddDays(-2).ToString("yyyyMMdd"), TO_DT = System.DateTime.Now.ToString("yyyyMMdd")
                    using (HttpResponseMessage response = await SystemProperties.PROGRAM_HTTP.PostAsync("p44011/mst", new StringContent(JsonConvert.SerializeObject(new PurVo() { FM_DT = System.DateTime.Now.AddMonths(-1).ToString("yyyyMMdd"), TO_DT = System.DateTime.Now.ToString("yyyyMMdd"), CLZ_FLG = "N", UPD_USR_ID = SystemProperties.USER, CHNL_CD = SystemProperties.USER_VO.CHNL_CD }), System.Text.Encoding.UTF8, "application/json")))
                    {
                        if (response.IsSuccessStatusCode)
                        {
