...
{
    public partial class P441103Detail_2Dialog : DXWindow
	{
        ...
        public P441103Detail_2Dialog(PurVo Dao)
		{
			InitializeComponent();
		...
		async void btn_ITEMS_Click(object sender, RoutedEventArgs e)
		{
            try
            {
			    ...
			    if (ITM_GRP_CLSS_CDVo != null)
			    {
                    ...
                    dao.SEEK = this.orgDao.PUR_ORD_NO;
                }
                //
                ...
                using (HttpResponseMessage response = await SystemProperties.PROGRAM_HTTP.PostAsync("p441103/popup/itm", new StringContent(JsonConvert.SerializeObject(dao), System.Text.Encoding.UTF8, "application/json")))
                {
                    if (response.IsSuccessStatusCode)
                    {
                        //IList<ItemCodeVo> resultItems = JsonConvert.DeserializeObject<IEnumerable<ItemCodeVo>>(await response.Content.ReadAsStringAsync()).Cast<ItemCodeVo>().ToList();
                        ...
                        //}
                        //this.ViewGridDtl.ItemsSource = this.SelectItems;
                        this.ViewGridDtl.ItemsSource = JsonConvert.DeserializeObject<IEnumerable<PurVo>>(await response.Content.ReadAsStringAsync()).Cast<PurVo>().ToList(); //this.SelectItems;
                        this.ViewGridDtl.RefreshData();
                    }
                }
        ...
        public async void SYSTEM_CODE_VO()
        {
            ...
            using (HttpResponseMessage response = await SystemProperties.PROGRAM_HTTP.GetAsync("s131/dtl/" + Properties.Settings.Default.SettingChnl + "/" + "L-001"))
            {
                if (response.IsSuccessStatusCode)
                {
                    ...
                    this.combo_ITM_GRP_CLSS_CD.SelectedIndex = 1;
                }
            }
