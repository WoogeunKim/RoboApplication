...
{
    //표준 공정 관리
    public sealed class M66410ViewModel : ViewModelBase, INotifyPropertyChanged
    {
        ...
        //
        private M66410MasterDialog masterDialog;

       
        public M66410ViewModel() 
        {
            Refresh();
        }
       ...
       public async void Refresh(string _KEY_CD = null)
        {
            try
            {
                using (HttpResponseMessage response = await SystemProperties.PROGRAM_HTTP.PostAsync("m66410", new StringContent(JsonConvert.SerializeObject(new ManVo() { CHNL_CD = SystemProperties.USER_VO.CHNL_CD }), System.Text.Encoding.UTF8, "application/json")))
                {
        ...
        {
            masterDialog = new M66410MasterDialog(new ManVo());
        ...
        public void EditMasterContact()
        {
            ...
            if (editDao != null)
            {
                masterDialog = new M66410MasterDialog(editDao);
        ...
        public async void DelMasterContact()
        {
            try
            {
                ...
                if (delDao != null)
                {
                    ...
                    {
                        using (HttpResponseMessage response = await SystemProperties.PROGRAM_HTTP.PostAsync("m66410/d", new StringContent(JsonConvert.SerializeObject(delDao), System.Text.Encoding.UTF8, "application/json")))
                        {
                            if (response.IsSuccessStatusCode)
                            {
                                int _Num = 0;
