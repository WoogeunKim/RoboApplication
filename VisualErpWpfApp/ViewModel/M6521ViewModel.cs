...
{
    //설비 관리
    public sealed class M6521ViewModel : ViewModelBase, INotifyPropertyChanged
    {

        private string _title = "생산계획/작업지시";
        ...
        public async void Refresh()
        {
            try
            {
                using (HttpResponseMessage response = await SystemProperties.PROGRAM_HTTP.PostAsync("m6521/mst", new StringContent(JsonConvert.SerializeObject(new ManVo() { CHNL_CD = SystemProperties.USER_VO.CHNL_CD, ROUT_CD = "IN", A_ROUT_CD = new string[] { "IN" } }), System.Text.Encoding.UTF8, "application/json")))
                {
        ...
        async void OnSelectedMasterItemChanged(string _LOT_DIV_NO = null)
        {
            try
            {
                ...
                using (HttpResponseMessage response = await SystemProperties.PROGRAM_HTTP.PostAsync("m6521/dtl", new StringContent(JsonConvert.SerializeObject(SelectedMasterItem), System.Text.Encoding.UTF8, "application/json")))
                {
                    if (response.IsSuccessStatusCode)
                    {
        ...
        public async void DelMasterContact()
        {
            try
            {
                ...
                if (delDao != null)
                {
                    MessageBoxResult result = WinUIMessageBox.Show("[" + delDao.LOT_DIV_NO + " / " + delDao.PROD_PLN_DT + "] 정말로 삭제 하시겠습니까?", _title, MessageBoxButton.YesNo, MessageBoxImage.Question);
                    if (result == MessageBoxResult.Yes)
                    {
                        //delDao.RN = x;
                        //delDao.CHNL_CD = SystemProperties.USER_VO.CHNL_CD;
                        using (HttpResponseMessage response = await SystemProperties.PROGRAM_HTTP.PostAsync("m6521/dtl/d", new StringContent(JsonConvert.SerializeObject(delDao), System.Text.Encoding.UTF8, "application/json")))
                        {
                            if (response.IsSuccessStatusCode)
                            {
                                int _Num = 0;
