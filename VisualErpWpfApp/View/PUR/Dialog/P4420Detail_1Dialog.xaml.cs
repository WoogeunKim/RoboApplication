...
{
    public partial class P4420Detail_1Dialog : DXWindow
	{
        ...
        private List<PurVo> SelectItems;

        public P4420Detail_1Dialog(PurVo Dao)
		{
			InitializeComponent();
		...
		async void btn_ITEMS_Click(object sender, RoutedEventArgs e)
		{
            try
            {
                ...
                //if (this.check_SCM_RLSE_CD.IsChecked == true)
                //{
                //    this.orgDao.IMP_PUR_NO = 

                //}

                using (HttpResponseMessage response = await SystemProperties.PROGRAM_HTTP.PostAsync("p4411/popup1", new StringContent(JsonConvert.SerializeObject(this.orgDao), System.Text.Encoding.UTF8, "application/json")))
                {
                    if (response.IsSuccessStatusCode)
                    {
                        this.SelectItems = JsonConvert.DeserializeObject<IEnumerable<PurVo>>(await response.Content.ReadAsStringAsync()).Cast<PurVo>().ToList();
		...
		private async void OKButton_Click(object sender, RoutedEventArgs e)
		{
            try
            {
                ...
                {
                    List<PurVo> _Items = (saveItems.FindAll(x => x.isCheckd == true)).ToList<PurVo>();
                    if (this.check_SCM_RLSE_CD.IsChecked == true)
                    {
                        //SCM 발주 저장
                        foreach (PurVo item in _Items)
                        {
                            item.SL_ORD_NO = (this.combo_SCM_RLSE_CD.SelectedItem as PurVo).IMP_PUR_NO;
                            item.SL_ORD_SEQ = (this.combo_SCM_RLSE_CD.SelectedItem as PurVo).IMP_PUR_SEQ;
                        }
                    }

                    //
                    int _Num = 0;
                    using (HttpResponseMessage response = await SystemProperties.PROGRAM_HTTP.PostAsync("p4411/dtl/i", new StringContent(JsonConvert.SerializeObject(_Items), System.Text.Encoding.UTF8, "application/json")))
                    {
                        if (response.IsSuccessStatusCode)
                        {
  ...
  //                  }
        ...
        {
            //this.combo_SCM_RLSE_CD.ItemsSource = SystemProperties.SYSTEM_CODE_VO("L-001");
            using (HttpResponseMessage response = await SystemProperties.PROGRAM_HTTP.PostAsync("p4411/popup2", new StringContent(JsonConvert.SerializeObject(new PurVo() { CHNL_CD = SystemProperties.USER_VO.CHNL_CD }), System.Text.Encoding.UTF8, "application/json")))
            {
                if (response.IsSuccessStatusCode)
                {
                    this.combo_SCM_RLSE_CD.ItemsSource = JsonConvert.DeserializeObject<IEnumerable<PurVo>>(await response.Content.ReadAsStringAsync()).Cast<PurVo>().ToList();
                    //this.combo_SCM_RLSE_CD.SelectedIndex = 0;
                }
            }
            btn_ITEMS_Click(null, null);
        }
