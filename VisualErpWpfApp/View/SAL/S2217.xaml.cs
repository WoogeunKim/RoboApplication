...
{
    public partial class S2217 : UserControl
    {
        ...
        public S2217()
        {
            DataContext = new S2217ViewModel();
            //
            InitializeComponent();
        ...
        //        ((S2217ViewModel)this.DataContext).setTitle();
        //    }
        ...
        private void GridColumn_Validate(object sender, DevExpress.Xpf.Grid.GridCellValidationEventArgs e)
        {
            try
            {
                ...
                bool slRtnItmQty = (e.Column.FieldName.ToString().Equals("SL_RTN_ITM_QTY") ? true : false);
                ...
                bool slRtnItmRmk = (e.Column.FieldName.ToString().Equals("SL_RTN_ITM_RMK") ? true : false);
                ...
                if (slRtnItmQty)
                {
                    if (e.IsValid)
                    {
                        ...
                        if (!masterDomain.SL_RTN_ITM_QTY.Equals((e.Value == null ? "" : e.Value.ToString())))
                        {
                            ...
                            masterDomain.SL_RTN_ITM_QTY = tmpInt;

                            try
                            {
                                masterDomain.SL_RTN_ITM_AMT = tmpInt * Convert.ToDouble(masterDomain.SL_ITM_PRC_DC_VAL);
                            }
                            catch (Exception)
                            {
                                masterDomain.SL_RTN_ITM_AMT = 0;
                            }
                ...
                else if (slRtnNm)
                {
                    if (e.IsValid)
                    {
                        if (string.IsNullOrEmpty(masterDomain.SL_RTN_NM + ""))
                        {
                            masterDomain.SL_RTN_NM = "";
                            masterDomain.SL_RTN_CD = "";
                        }
                        //
                        if (!masterDomain.SL_RTN_NM.Equals((e.Value == null ? "" : e.Value.ToString())))
                        {
                            SystemCodeVo bankIoDao = this.lue_SL_RTN_NM.GetItemFromValue(e.Value) as SystemCodeVo;
                            //
                            if (bankIoDao != null)
                            {
                ...
                else if (slBadItmNm)
                {
                    if (e.IsValid)
                    {
                        if (string.IsNullOrEmpty(masterDomain.SL_BAD_ITM_NM + ""))
                        {
                            masterDomain.SL_BAD_ITM_NM = "";
                            masterDomain.SL_BAD_ITM_CD = "";
                        }
                        //
                        if (!masterDomain.SL_BAD_ITM_NM.Equals((e.Value == null ? "" : e.Value.ToString())))
                        {
                            SystemCodeVo bankIoDao = this.lue_SL_BAD_ITM_NM.GetItemFromValue(e.Value) as SystemCodeVo;
                            //
                            if (bankIoDao != null)
                            {
                ...
                else if (slRtnItmRmk)
                {
                    if (e.IsValid)
                    {
                        if (string.IsNullOrEmpty(masterDomain.SL_RTN_ITM_RMK))
                        {
                        ...
                        {
                            masterDomain.SL_RTN_ITM_RMK = e.Value.ToString();
                            masterDomain.isCheckd = true;
                        }
                    }
                }
