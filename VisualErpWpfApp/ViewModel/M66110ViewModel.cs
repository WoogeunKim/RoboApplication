...
{
    public sealed class M66110ViewModel : ViewModelBase, INotifyPropertyChanged
    {
        private string title = "휴무일관리";
        ...
        private M66110MasterDialog masterDialog;
        ...
        [Command]
        public async void Refresh(string _EXPT_DT = null)
        {
            try
            {
                ...
                using (HttpResponseMessage response = await SystemProperties.PROGRAM_HTTP.PostAsync("M66110", new StringContent(JsonConvert.SerializeObject(_param), System.Text.Encoding.UTF8, "application/json")))
                {
                    ...
                    if (SelectMstList.Count >= 1)
                    {
                        ...
                        if (string.IsNullOrEmpty(_EXPT_DT))
                        {
                            SelectedMstItem = SelectMstList[0];
                        }
                        else
                        {
                            SelectedMstItem = SelectMstList.Where(x => x.EXPT_DT.Equals(_EXPT_DT)).LastOrDefault<ManVo>();
                        }
                    }
                    else
                    {
        ...
        public void NewContact()
        {
            if (SelectedDates.Count > 0)
            {
                ...
                masterDialog = new M66110MasterDialog(datesList);
        ...
        public void EditDtlContact()
        {
            ...
            masterDialog = new M66110MasterDialog(SelectedMstItem);
            masterDialog.Title = title + " - 수정(" + SelectedMstItem.EXPT_DT + ")";
            masterDialog.Owner = Application.Current.MainWindow;
            masterDialog.BorderEffect = BorderEffect.Default;
            masterDialog.BorderEffectActiveColor = new SolidColorBrush(Color.FromRgb(255, 128, 0));
            masterDialog.BorderEffectInactiveColor = new SolidColorBrush(Color.FromRgb(255, 170, 170));
            ...
            if (isDialog)
            {
                Refresh(masterDialog.resultDomain.EXP_DT);
            }
        }
