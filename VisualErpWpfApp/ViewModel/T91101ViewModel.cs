...
{
    public sealed class T91101ViewModel : ViewModelBase, INotifyPropertyChanged
    {
        ...
        private T91101MasterDialog detailDialog;
        ...
        public void NewContact()
        {
            if (SelectedMstItems.Count > 0)
            {
              //  string _coNm = this.SelectedMstItems[0].LOT_NO;
                ...
                detailDialog = new T91101MasterDialog(this.SelectedMstItems);
        ...
        public async void BadRefresh(object _obj)
        {
            try
            {
                ...
                if (this.isPRNT_PLN == true)
                {
                    ...
                    using (HttpResponseMessage response = await SystemProperties.PROGRAM_HTTP.PostAsync("t91101/bad", new StringContent(JsonConvert.SerializeObject(Dao), System.Text.Encoding.UTF8, "application/json")))
                    {
                        if (response.IsSuccessStatusCode)
                        {
