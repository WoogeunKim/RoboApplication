...
{
    public partial class S2222MasterDialog : DXWindow
    {
        ...
        public S2222MasterDialog(SaleVo Dao)
        {
            InitializeComponent();
            ...
            {
                CLT_BIL_DELT_NO = Dao.CLT_BIL_DELT_NO,
                CLT_BIL_DELT_DT = Dao.CLT_BIL_DELT_DT,
                AREA_CD = Dao.AREA_CD,
                AREA_NM = Dao.AREA_NM,
            ...
            else
            {
                ...
                this.text_CLT_BIL_DELT_NO.IsReadOnly = true;
                this.isEdit = false;
                copyDao.CLT_BIL_DELT_DT = System.DateTime.Now.ToString("yyyy-MM-dd");
            ...
            this.text_CLT_BIL_DELT_DT.Focus();
        }
        ...
        private async void OKButton_Click(object sender, RoutedEventArgs e)
        {
            if (ValueCheckd())
            {
                ...
                if (isEdit == false)
                {
                    ...
                    using (HttpResponseMessage response = await SystemProperties.PROGRAM_HTTP.PostAsync("s2222/mst/i", new StringContent(JsonConvert.SerializeObject(this.updateDao), System.Text.Encoding.UTF8, "application/json")))
                    {
                        if (response.IsSuccessStatusCode)
                        {
        ...
        public Boolean ValueCheckd()
        {
            ...
            //}
            //else 
            if (string.IsNullOrEmpty(this.text_CLT_BIL_DELT_DT.Text))
            {
                WinUIMessageBox.Show("[상계 일자] 입력 값이 맞지 않습니다.", "[유효검사]" + title, MessageBoxButton.OK, MessageBoxImage.Warning);
                this.text_CLT_BIL_DELT_DT.IsTabStop = true;
                this.text_CLT_BIL_DELT_DT.Focus();
                return false;
            }
        ...
        private SaleVo getDomain()
        {
            ...
            Dao.CLT_BIL_DELT_DT = Convert.ToDateTime(this.text_CLT_BIL_DELT_DT.Text).ToString("yyyy-MM-dd");
