...
{
    public sealed class P441102ViewModel : ViewModelBase, INotifyPropertyChanged
    {
        ...
        private P441102MasterDialog masterDialog;
        private P441102DetailDialog detailDialog;
        ...
        public async void Refresh(string _PUR_ORD_NO = null)
        {
            try
            {
                ...
                using (HttpResponseMessage response = await SystemProperties.PROGRAM_HTTP.PostAsync("p441102/mst", new StringContent(JsonConvert.SerializeObject(new PurVo() { FM_DT = (StartDt).ToString("yyyy-MM-dd"), TO_DT = (EndDt).ToString("yyyy-MM-dd"), AREA_CD = M_SL_AREA_NM.CLSS_CD, CHNL_CD = SystemProperties.USER_VO.CHNL_CD, PUR_ITM_CD = "M" }), System.Text.Encoding.UTF8, "application/json")))
                {
                    ...
                    Title = "[기간]" + (StartDt).ToString("yyyy-MM-dd") + "~" + (EndDt).ToString("yyyy-MM-dd") + ",   [사업장]" + M_SL_AREA_NM.CLSS_DESC + ",   [발주 품목]원자재";
        ...
        public async void SelectMstDetail()
        {
             try
             {
                ...
                using (HttpResponseMessage response = await SystemProperties.PROGRAM_HTTP.PostAsync("p441102/dtl", new StringContent(JsonConvert.SerializeObject(SelectedMstItem), System.Text.Encoding.UTF8, "application/json")))
                {
                    if (response.IsSuccessStatusCode)
                    {
        ...
        {
            masterDialog = new P441102MasterDialog(new PurVo() { AREA_CD = M_SL_AREA_NM.CLSS_CD, AREA_NM = M_SL_AREA_NM.CLSS_DESC, PUR_DT = System.DateTime.Now.ToString("yyyy-MM-dd"), PUR_CLZ_FLG = "N", PUR_EMPE_ID = SystemProperties.USER_VO.USR_ID });
        ...
        public void EditContact()
        {
            ...
            masterDialog = new P441102MasterDialog(SelectedMstItem);
            ...
            try
            {
                ...
                if (delDao != null)
                {
                    ...
                    if (result == MessageBoxResult.Yes)
                    {
                        ...
                        using (HttpResponseMessage response = await SystemProperties.PROGRAM_HTTP.PostAsync("p441102/mst/d", new StringContent(JsonConvert.SerializeObject(SelectedMstItem), System.Text.Encoding.UTF8, "application/json")))
                        {
                            if (response.IsSuccessStatusCode)
                            {
                                int _Num = 0;
        ...
        public async void PrintTagContact(string _obj)
        {
            try
            {
                ...
                P441102TagReport report;

                //출력 수
                if (IS_SALF_PRN == true)
                {
                    if (string.IsNullOrEmpty(M_BAR_TEXT))
                    {
                        return;
                    }
                    else if (Convert.ToInt32(M_BAR_TEXT) <= 0)
                    ...
                    if (_obj.Equals("A"))
                        {
                            MessageBoxResult result = WinUIMessageBox.Show("현품표(A5) [" + SelectedMstItem.PUR_ORD_NO + "] → [출력 수 : " + M_BAR_TEXT + "]" + " (전체 현품표) 생성 하시겠습니까?", _title, MessageBoxButton.YesNo, MessageBoxImage.Question);
                            if (result == MessageBoxResult.Yes)
                            {
                                //바코드 출력
                                using (HttpResponseMessage response = await SystemProperties.PROGRAM_HTTP.PostAsync("p441102/dtl/barcode", new StringContent(JsonConvert.SerializeObject(new PurVo() { CHNL_CD = SystemProperties.USER_VO.CHNL_CD, PUR_ORD_NO = SearchDetail.PUR_ORD_NO, RN = Convert.ToInt32(M_BAR_TEXT), R_MM_11 = "SALF" }), System.Text.Encoding.UTF8, "application/json")))
                                {
                                    if (response.IsSuccessStatusCode)
                                    {
                                        ...
                                        report = new P441102TagReport(_barCodeList);
                        ...
                        {
                            MessageBoxResult result = WinUIMessageBox.Show("현품표(A5) [" + SearchDetail.PUR_ORD_NO + "/" + SearchDetail.PUR_ORD_SEQ + "] → [출력 수 : " + M_BAR_TEXT + "]" + " (개별 현품표) 생성 하시겠습니까?", _title, MessageBoxButton.YesNo, MessageBoxImage.Question);
                            if (result == MessageBoxResult.Yes)
                            {

                                SearchDetail.RN = Convert.ToInt32(M_BAR_TEXT);
                                //SearchDetail.SL_ITM_QTY = 0;
                                ...
                                using (HttpResponseMessage response = await SystemProperties.PROGRAM_HTTP.PostAsync("p441102/dtl/barcode", new StringContent(JsonConvert.SerializeObject(SearchDetail), System.Text.Encoding.UTF8, "application/json")))
                                {
                                    if (response.IsSuccessStatusCode)
                                    {
                                        ...
                                        window.PreviewControl.DocumentSource = report;
                                        report.CreateDocument(true);
                                        window.Title = "현품표(A5) [" + SearchDetail.PUR_ORD_NO + "/" + SearchDetail.PUR_ORD_SEQ + "] ";
                                        window.Owner = Application.Current.MainWindow;
                                        window.ShowDialog();
                                    }
                                }
                ...
                else if (this.IS_AUTO_PRN == true) 
                {
                    ...
                    if (_obj.Equals("A"))
                    {
                        MessageBoxResult result = WinUIMessageBox.Show("현품표(A5) [" + SelectedMstItem.PUR_ORD_NO + "]  (전체 현품표) 생성 하시겠습니까?", _title + " - [포장 단위]", MessageBoxButton.YesNo, MessageBoxImage.Question);
                        if (result == MessageBoxResult.Yes)
                        {
                ...
                {
                        MessageBoxResult result = WinUIMessageBox.Show("현품표(A5) [" + SearchDetail.PUR_ORD_NO + "/" + SearchDetail.PUR_ORD_SEQ + "]  (개별 현품표) 생성 하시겠습니까?", _title + " - [포장 단위]", MessageBoxButton.YesNo, MessageBoxImage.Question);
                        if (result == MessageBoxResult.Yes)
                        {
        ...
        public async void PrintContact()
        {
            try
            {
                ...
                P441102Report report = new P441102Report(SelectDtlList);
                ...
                window.PreviewControl.DocumentSource = report;
                report.CreateDocument(true);
                window.Title = "원재료 발주서 [" + SelectedMstItem.PUR_ORD_NO + "] ";
                window.Owner = Application.Current.MainWindow;
                window.ShowDialog();
        ...
        public void NewDtlContact()
        {
            ...
            detailDialog = new P441102DetailDialog(new PurVo() { PUR_ORD_NO = SelectedMstItem.PUR_ORD_NO, CO_CD = SelectedMstItem.PUR_CO_CD, CHNL_CD = SystemProperties.USER_VO.CHNL_CD, CRE_USR_ID = SystemProperties.USER_VO.USR_ID, UPD_USR_ID = SystemProperties.USER_VO.USR_ID });
            detailDialog.Title = "발주 자재 관리 - " + SelectedMstItem.CO_NM + " / 원자재 소요량 전개[" + SelectedMstItem.PUR_ORD_NO + "]";
        ...
        public async void DelDtlContact()
        {
            try
            {
                ...
                {
                    SelectedMstItem.CHNL_CD = SystemProperties.USER_VO.CHNL_CD;
                    using (HttpResponseMessage response = await SystemProperties.PROGRAM_HTTP.PostAsync("p441102/dtl/d", new StringContent(JsonConvert.SerializeObject(SearchDetail), System.Text.Encoding.UTF8, "application/json")))
                    {
                        if (response.IsSuccessStatusCode)
                        {
                            int _Num = 0;
        ...
        public void EmailContact()
        {
            ...
            emailDialog = new P441102EmailDialog(SelectedMstItem);
