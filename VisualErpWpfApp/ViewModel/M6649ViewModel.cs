...
{
    //표준 공정 관리
    public sealed class M6649ViewModel : ViewModelBase, INotifyPropertyChanged
    {
        private string _title = "예방 보전계획 등록";
        ...
        //
        private M6649MasterDialog masterDialog;

       
        public M6649ViewModel() 
        {
            Refresh();
        }
       ...
       public async void Refresh(string _KEY_CD = null)
        ...
        {
            masterDialog = new M6649MasterDialog(new ManVo());
        ...
        public void EditMasterContact()
        {
            ...
            if (editDao != null)
            {
                masterDialog = new M6649MasterDialog(editDao);
        ...
        public async void DelMasterContact()
        {
            try
            {
                ...
                if (delDao != null)
                {
                    ...
                    {
                        using (HttpResponseMessage response = await SystemProperties.PROGRAM_HTTP.PostAsync("m6649/d", new StringContent(JsonConvert.SerializeObject(delDao), System.Text.Encoding.UTF8, "application/json")))
                        {
                            if (response.IsSuccessStatusCode)
                            {
                                int _Num = 0;
