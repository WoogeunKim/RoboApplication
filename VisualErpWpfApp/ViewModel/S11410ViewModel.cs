...
{
    public sealed class S11410ViewModel : ViewModelBase, INotifyPropertyChanged
    {
        private string title = "품목군관리";
        ...
        private S11410MasterDialog masterDialog;
        ...
        [Command]
        public async void Refresh(string _CAR_TP_CD = null)
        {
            try
            {
                using (HttpResponseMessage response = await SystemProperties.PROGRAM_HTTP.PostAsync("s11410", new StringContent(JsonConvert.SerializeObject(new SaleVo() { CHNL_CD = SystemProperties.USER_VO.CHNL_CD }), System.Text.Encoding.UTF8, "application/json")))
                {
                    ...
                    if (SelectMstList.Count >= 1)
                    {
                        ...
                        if (string.IsNullOrEmpty(_CAR_TP_CD))
                        {
                            SelectedMstItem = SelectMstList[0];
                        }
                        else
                        {
                            SelectedMstItem = SelectMstList.Where(x => x.CAR_TP_CD.Equals(_CAR_TP_CD)).LastOrDefault<SaleVo>();
                        }
                    }
                    else
                    {
        ...
        {
            masterDialog = new S11410MasterDialog(new SaleVo());
            ...
            if (isDialog)
            {
                Refresh(masterDialog.ResultDao.CAR_TP_CD);
            }
        }
        ...
        public void EditDtlContact()
        {
            ...
            masterDialog = new S11410MasterDialog(SelectedMstItem);
            masterDialog.Title = title + " - 수정(" + SelectedMstItem.CAR_TP_CD + " / " + SelectedMstItem.CAR_TP_NM + ")";
            masterDialog.Owner = Application.Current.MainWindow;
            masterDialog.BorderEffect = BorderEffect.Default;
            masterDialog.BorderEffectActiveColor = new SolidColorBrush(Color.FromRgb(255, 128, 0));
            masterDialog.BorderEffectInactiveColor = new SolidColorBrush(Color.FromRgb(255, 170, 170));
        ...
        public async void DelContact()
        {
            try
            {
                ...
                if (SelectedMstItem != null)
                {
                    MessageBoxResult result = WinUIMessageBox.Show(SelectedMstItem.CAR_TP_CD + "/" + SelectedMstItem.CAR_TP_NM + " 정말로 삭제 하시겠습니까?", title, MessageBoxButton.YesNo, MessageBoxImage.Question);
                    if (result == MessageBoxResult.Yes)
                    {
                        SelectedMstItem.DELT_FLG = "Y";

                        using (HttpResponseMessage response = await SystemProperties.PROGRAM_HTTP.PostAsync("s11410/u", new StringContent(JsonConvert.SerializeObject(SelectedMstItem), System.Text.Encoding.UTF8, "application/json")))
                        {
                            if (response.IsSuccessStatusCode)
                            {
                                int _Num = 0;
