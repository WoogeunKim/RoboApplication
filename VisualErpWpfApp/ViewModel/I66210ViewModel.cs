...
{
    public sealed class I66210ViewModel : ViewModelBase, INotifyPropertyChanged
    {
        ...
        private I66210DetailOutDialog detailOutDialog;
       ...
       public async void Refresh()
        {
            try
            {
               ...
               // using (HttpResponseMessage response = await SystemProperties.PROGRAM_HTTP.PostAsync("i66110/mst", new StringContent(JsonConvert.SerializeObject(new InvVo() {CHNL_CD = SystemProperties.USER_VO.CHNL_CD, FM_DT = (StartDt).ToString("yyyy-MM-dd"), TO_DT = (EndDt).ToString("yyyy-MM-dd"), AREA_CD = M_SL_AREA_NM.CLSS_CD, GBN = (string.IsNullOrEmpty(M_CHECKD) ? null : M_CHECKD), LOC_CD = M_SL_LOC_NM.CLSS_CD, IN_FLG = (IN_FLG == true ? "Y" : "N"), IO_CD = "O" }), System.Text.Encoding.UTF8, "application/json")))
                using (HttpResponseMessage response = await SystemProperties.PROGRAM_HTTP.PostAsync("i66210/mst", new StringContent(JsonConvert.SerializeObject(new InvVo() {CHNL_CD = SystemProperties.USER_VO.CHNL_CD, FM_DT = (StartDt).ToString("yyyy-MM-dd"), TO_DT = (EndDt).ToString("yyyy-MM-dd"), AREA_CD = M_SL_AREA_NM.CLSS_CD, IO_CD = "O" }), System.Text.Encoding.UTF8, "application/json")))
                {
        ...
        public void NewOutContact()
        {
            try
            {
                ...
                //detailOutDialog = new I66210DetailOutDialog(SelectedMstItem);
                detailOutDialog = new I66210DetailOutDialog();
                //detailOutDialog.Title = "품목 출고 - " + SelectedMstItem.INSRL_NO;
                detailOutDialog.Title = "품목 출고";
        ...
        public async void DelContact()
        {
            try
            {
                ...
                MessageBoxResult result = WinUIMessageBox.Show("[수불 번호 :" + SelectedMstItem.INSRL_NO + "] 정말로 삭제 하시겠습니까?", "[삭제]" + _title, MessageBoxButton.YesNo, MessageBoxImage.Question);
                if (result == MessageBoxResult.Yes)
                {
                    int _Num = 0;
                    using (HttpResponseMessage response = await SystemProperties.PROGRAM_HTTP.PostAsync("i66110/mst/d", new StringContent(JsonConvert.SerializeObject(SelectedMstItem), System.Text.Encoding.UTF8, "application/json")))
                    {
                        if (response.IsSuccessStatusCode)
                        {
        ...
        //    try
        //    {
        //        detailOutDialog = new I66110DetailPurDialog(new InvVo() {CHNL_CD = SystemProperties.USER_VO.CHNL_CD, FM_DT = (StartDt).ToString("yyyy-MM-dd"), TO_DT = (EndDt).ToString("yyyy-MM-dd") });
        //        detailOutDialog.Title = "발주 입고";
        //        detailOutDialog.Owner = Application.Current.MainWindow;
        //        detailOutDialog.BorderEffect = BorderEffect.Default;
        //        detailOutDialog.BorderEffectActiveColor = new SolidColorBrush(Color.FromRgb(255, 128, 0));
        //        detailOutDialog.BorderEffectInactiveColor = new SolidColorBrush(Color.FromRgb(255, 170, 170));
        ...
        //[Command]
        //public void NewDtlOthContact()
        //{
        //    try
        //    {
        //        detailOthDialog = new I66110DetailOthDialog(new InvVo() { CHNL_CD = SystemProperties.USER_VO.CHNL_CD, FM_DT = (StartDt).ToString("yyyy-MM-dd"), TO_DT = (EndDt).ToString("yyyy-MM-dd") });
        //        detailOthDialog.Title = "기타 입고";
        //        detailOthDialog.Owner = Application.Current.MainWindow;
        //        detailOthDialog.BorderEffect = BorderEffect.Default;
        //        detailOthDialog.BorderEffectActiveColor = new SolidColorBrush(Color.FromRgb(255, 128, 0));
        //        detailOthDialog.BorderEffectInactiveColor = new SolidColorBrush(Color.FromRgb(255, 170, 170));
        //        bool isDialog = (bool)detailOthDialog.ShowDialog();
        //        //if (isDialog)
        //        {
        //            Refresh();
        //        }
