...
{
    public sealed class S2221ViewModel : ViewModelBase, INotifyPropertyChanged
    {
        ...
        [Command]
        public async void Refresh(string _CLT_BIL_NO = null)
        {
            try
            {
                ...
                using (HttpResponseMessage response = await SystemProperties.PROGRAM_HTTP.PostAsync("s2221/mst", new StringContent(JsonConvert.SerializeObject(_param), System.Text.Encoding.UTF8, "application/json")))
                {
        ...
        {
            masterDialog = new S2221MasterDialog(new SaleVo() { AREA_NM = M_SL_AREA_NM.CLSS_DESC, AREA_CD = M_SL_AREA_NM.CLSS_CD, CLT_BIL_DT = DateTime.Now.ToString("yyyy-MM-dd") });
