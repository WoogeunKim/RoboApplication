...
{
    //설비 관리
    public sealed class M6730ViewModel : ViewModelBase, INotifyPropertyChanged
    {

        private string _title = "조립작업실적입력";
        ...
        public async void Refresh()
        {
            try
            {
                ...
                using (HttpResponseMessage response = await SystemProperties.PROGRAM_HTTP.PostAsync("m6730/", new StringContent(JsonConvert.SerializeObject(new ManVo() { FM_DT = (StartDt).ToString("yyyy-MM-dd"), TO_DT = (EndDt).ToString("yyyy-MM-dd"), CHNL_CD = SystemProperties.USER_VO.CHNL_CD }), System.Text.Encoding.UTF8, "application/json")))
                {
       ...
       public async void MstRefresh(string _LOT_DIV_NO = null)
        {
            try
            {
                ...
                using (HttpResponseMessage response = await SystemProperties.PROGRAM_HTTP.PostAsync("m6730/mst", new StringContent(JsonConvert.SerializeObject(SelectedItem), System.Text.Encoding.UTF8, "application/json")))
                {
                    if (response.IsSuccessStatusCode)
                    {
        ...
        private string _PANEL_TITLE_3 = "투입자재 불량 현황";
        ...
        async void OnSelectedMasterItemChanged()
        {
            try
            {
                ...
                //불량 현황
                using (HttpResponseMessage response = await SystemProperties.PROGRAM_HTTP.PostAsync("m6730/dtl2", new StringContent(JsonConvert.SerializeObject(this.SelectedMasterItem), System.Text.Encoding.UTF8, "application/json")))
                {
                    if (response.IsSuccessStatusCode)
                    {
                ...
                //비가동 현황
                using (HttpResponseMessage response = await SystemProperties.PROGRAM_HTTP.PostAsync("m6730/dtl1", new StringContent(JsonConvert.SerializeObject(this.SelectedMasterItem), System.Text.Encoding.UTF8, "application/json")))
                {
                    if (response.IsSuccessStatusCode)
                    {
                ...
                //투입 자재 - 불량 현황
                using (HttpResponseMessage response = await SystemProperties.PROGRAM_HTTP.PostAsync("m6730/dtl3/mst", new StringContent(JsonConvert.SerializeObject(this.SelectedMasterItem), System.Text.Encoding.UTF8, "application/json")))
                {
                    if (response.IsSuccessStatusCode)
                    {
                        ...
                        this.PANEL_TITLE_3 = "투입자재 불량 현황(" + this.SelectedPopup3ViewList.Count + ")";
                    }
        ...
        public async void AddContact()
        {
            try
            {
                ...
                MessageBoxResult result = WinUIMessageBox.Show("(지시 일자 : " + System.DateTime.Now.ToString("yyyy-MM-dd HH:mm") + ") 조립 추가 하시겠습니까?", _title, MessageBoxButton.YesNo, MessageBoxImage.Question);
                if (result == MessageBoxResult.Yes)
                {
                    using (HttpResponseMessage response = await SystemProperties.PROGRAM_HTTP.PostAsync("m6730/mst/m", new StringContent(JsonConvert.SerializeObject(new List<ManVo>() { new ManVo() { UPD_USR_ID = SystemProperties.USER, CRE_USR_ID = SystemProperties.USER, CHNL_CD = SystemProperties.USER_VO.CHNL_CD, LOT_DIV_NO = this.SelectedItem.LOT_DIV_NO, ROUT_CD = this.SelectedItem.ROUT_CD, EQ_NO = this.SelectedItem.EQ_NO, MOLD_CD = this.SelectedItem.MOLD_CD, ITM_MOLD_SEQ = this.SelectedItem.ITM_MOLD_SEQ } }), System.Text.Encoding.UTF8, "application/json")))
                    {
                        int _Num;
                        if (response.IsSuccessStatusCode)
                        {
        ...
        public async void DelContact()
        {
            try
            {
                ...
                if (delDao != null)
                {
                    MessageBoxResult result = WinUIMessageBox.Show("(" + delDao.RN + " / " + delDao.PROD_PLN_DT + " / " + delDao.LOT_DIV_NO + ")  정말로 삭제 하시겠습니까?", _title, MessageBoxButton.YesNo, MessageBoxImage.Question);
                    if (result == MessageBoxResult.Yes)
                    {

                        using (HttpResponseMessage response = await SystemProperties.PROGRAM_HTTP.PostAsync("m6730/mst/d", new StringContent(JsonConvert.SerializeObject(delDao), System.Text.Encoding.UTF8, "application/json")))
                        {
                            if (response.IsSuccessStatusCode)
                            {
                                int _Num = 0;
        ...
        public async void DelMasterContact()
        {
            try
            {
                ...
                if (delDao != null)
                {
                    MessageBoxResult result = WinUIMessageBox.Show("(" + delDao.PROD_PLN_DT + " / " + delDao.LOT_DIV_NO + " / " + delDao.ROUT_NM + ")  정말로 삭제 하시겠습니까?", _title, MessageBoxButton.YesNo, MessageBoxImage.Question);
                    if (result == MessageBoxResult.Yes)
                    {
