...
{
    public sealed class P441100ViewModel : ViewModelBase, INotifyPropertyChanged
    {

        private P441100MasterDialog masterDialog;
        private P441100DetailDialog detailDialog;

        private P441100MergeDialog mergeDialog;
          ...
          public async void Refresh(string _SL_ORD_NO = null)
          {
              ...
              try
              {
                  ...
                  //SelectDtlList = null;
                    ...
                    using (HttpResponseMessage response = await SystemProperties.PROGRAM_HTTP.PostAsync("p441100/mst", new StringContent(JsonConvert.SerializeObject(_param), System.Text.Encoding.UTF8, "application/json")))
                    {
              ...
              catch (System.Exception eLog)
              {
                ...
                WinUIMessageBox.Show(eLog.Message, "[" + SystemProperties.PROGRAM_TITLE + "]원자재 소요량 전개", MessageBoxButton.OK, MessageBoxImage.Error, MessageBoxResult.None, MessageBoxOptions.None);
                  return;
              }
        ...
        public async void Merge()
        {
            try
            {
                ...
                this.mergeDialog = new P441100MergeDialog(SelectMstList);
                this.mergeDialog.Title = "원자재 소요량 전개 - 소요량 취합등록";
        ...
        private string _M_PUR_CLZ_FLG = "Y";
          ...
          public PurVo SelectedMstItem
          {
              ...
              set
              {
                  if (value != null)
                  {
                      SetProperty(ref _selectedMstItem, value, () => SelectedMstItem, onSelBulkDtl);
                  }
              }
          }

        public async void onSelBulkDtl()
        {
            try
            {
                if (SelectedMstItem == null)
                ...
                else if (string.IsNullOrEmpty(SelectedMstItem.UN_FOL_NO))
                {
                    SelectBulkDtlList = null;
                    SelectDtlList = null;
                    return;
                }


                using (HttpResponseMessage response = await SystemProperties.PROGRAM_HTTP.PostAsync("p441100/bulk", new StringContent(JsonConvert.SerializeObject(SelectedMstItem), System.Text.Encoding.UTF8, "application/json")))
                {
                    if (response.IsSuccessStatusCode)
                    {
                        SelectBulkDtlList = JsonConvert.DeserializeObject<IEnumerable<PurVo>>(await response.Content.ReadAsStringAsync()).Cast<PurVo>().ToList();
                        if (SelectBulkDtlList.Count > 0)
                        {
                            SelectedBulkDtlItem = SelectBulkDtlList[0];
                        }
                    }
                }
            }
        ...
        public async void onSelDtl()
        {
            try
            {
                if (SelectedBulkDtlItem == null)
                {
                    return;
                }


                using (HttpResponseMessage response = await SystemProperties.PROGRAM_HTTP.PostAsync("p441100/dtl", new StringContent(JsonConvert.SerializeObject(SelectedBulkDtlItem), System.Text.Encoding.UTF8, "application/json")))
                {
                    if (response.IsSuccessStatusCode)
                    {
        ...
        //Dtl - Bulk
        public IList<PurVo> SelectBulkDtlList
        {
            get { return selectedBulkDtlList; }
            set { SetProperty(ref selectedBulkDtlList, value, () => SelectBulkDtlList); }
        }

        PurVo _selectedBulkDtlItem;
        public PurVo SelectedBulkDtlItem
        {
            get
            {
                return _selectedBulkDtlItem;
            }
            set
            {
                if (value != null)
                {
                    SetProperty(ref _selectedBulkDtlItem, value, () => SelectedBulkDtlItem, onSelDtl);
                }
            }
        }
        ...
        public async void Ok()
        {
            try
            {
                ...
                //
                MessageBoxResult result = WinUIMessageBox.Show("[" +SelectedMstItem.SL_ORD_NO + "/" + SelectedMstItem.SL_ORD_SEQ + "] " + SelectedMstItem.ITM_NM + " 소요량 전개 하시겠습니까?", "원자재 소요량 전개", MessageBoxButton.YesNo, MessageBoxImage.Question);
                if (result == MessageBoxResult.Yes)
                {
                    int _Num = 0;
                    string resultMsg = "";
                    string _SL_ORD_NO = SelectedMstItem.SL_ORD_NO + "_" + SelectedMstItem.SL_ORD_SEQ;
                    using (HttpResponseMessage response = await SystemProperties.PROGRAM_HTTP.PostAsync("p441100/proc", new StringContent(JsonConvert.SerializeObject(SelectedMstItem), System.Text.Encoding.UTF8, "application/json")))
                    {
                        if (response.IsSuccessStatusCode)
                        {
                            ...
                            {
                                //실패
                                WinUIMessageBox.Show(resultMsg, "원자재 소요량 전개", MessageBoxButton.OK, MessageBoxImage.Error);
                                return;
                            }
                            ...
                            //성공
                            WinUIMessageBox.Show("완료되었습니다.", "원자재 소요량 전개", MessageBoxButton.OK, MessageBoxImage.Information, MessageBoxResult.None, MessageBoxOptions.None);
                        }
                    }
                }
            }
        ...
        public async void LssVal()
        {
            try
            {
                ...
                this.detailDialog = new P441100DetailDialog(SelectDtlList);
                this.detailDialog.Title = "총 Loss (자동계산)";
                this.detailDialog.WindowStartupLocation = WindowStartupLocation.CenterScreen;
                this.detailDialog.Owner = Application.Current.MainWindow;
                this.detailDialog.BorderEffect = BorderEffect.Default;
                //masterDialog.BorderEffectActiveColor = new SolidColorBrush(Color.FromRgb(255, 128, 0));
                //masterDialog.BorderEffectInactiveColor = new SolidColorBrush(Color.FromRgb(255, 170, 170));
                bool isDialog = (bool)this.detailDialog.ShowDialog();
                if (isDialog)
                {

                    //
                    MessageBoxResult result = WinUIMessageBox.Show("[" + SelectedMstItem.SL_ORD_NO + "/" + SelectedMstItem.SL_ORD_SEQ + "] " + SelectedMstItem.UN_FOL_NO + " Loss (자동계산) 하시겠습니까?", "원자재 소요량 전개 - [총 Loss : " + this.detailDialog.text_LSS_VAL.Text + "]", MessageBoxButton.YesNo, MessageBoxImage.Question);
                    if (result == MessageBoxResult.Yes)
                    {
                        SelectedMstItem.UPD_USR_ID = SystemProperties.USER;
                        SelectedMstItem.LSS_VAL = this.detailDialog.text_LSS_VAL.Text;

                        int _Num = 0;
                        string resultMsg = "";
                        using (HttpResponseMessage response = await SystemProperties.PROGRAM_HTTP.PostAsync("p441100/dtl/loss/u", new StringContent(JsonConvert.SerializeObject(SelectedMstItem), System.Text.Encoding.UTF8, "application/json")))
                        {
                            if (response.IsSuccessStatusCode)
                            {
                                resultMsg = await response.Content.ReadAsStringAsync();
        ...
        public async void Custemer()
        {
            try
            {
                ...
                this.masterDialog = new P441100MasterDialog(SelectedDtlItem);
                //조회
                ...
                this.masterDialog.Title = "원자재 소요량 전개 - 거래처";
                ...
                if (isDialog)
                {
                    try
                    {
                        ...
                        int _Num = 0;
                        string resultMsg = "";
                        using (HttpResponseMessage response = await SystemProperties.PROGRAM_HTTP.PostAsync("p441100/dtl/u", new StringContent(JsonConvert.SerializeObject(SelectedDtlItem), System.Text.Encoding.UTF8, "application/json")))
                        {
                            if (response.IsSuccessStatusCode)
                            {
                                resultMsg = await response.Content.ReadAsStringAsync();
                                ...
                                {
                                    //실패
                                    WinUIMessageBox.Show(resultMsg, "[" + SystemProperties.PROGRAM_TITLE + "]원자재 소요량 전개", MessageBoxButton.OK, MessageBoxImage.Error, MessageBoxResult.None, MessageBoxOptions.None);
                                    return;
                                }
                            }
                        }
