...
{
    public sealed class T91104ViewModel : ViewModelBase, INotifyPropertyChanged
    {
        private string _title = "협력업체 반품";
        ...
        private IList<TecVo> selectedPopupViewList = new List<TecVo>();
        

        //private T91100MasterDialog detailDialog;
       ...
       public async void Refresh()
        ...
        public IList<TecVo> SelectedPopupViewList
        {
            get { return selectedPopupViewList; }
            set { SetProperty(ref selectedPopupViewList, value, () => SelectedPopupViewList); }
        }
        ...
        public async void NewContact()
        {
            ...
            {
                MessageBoxResult result = WinUIMessageBox.Show("[총 : " + SelectedMstItems.Count + "] 외주 불량 반납 하시겠습니까?",  _title, MessageBoxButton.YesNo, MessageBoxImage.Question);
                if (result == MessageBoxResult.Yes)
                {
                    int _Num = 0;
                    using (HttpResponseMessage response = await SystemProperties.PROGRAM_HTTP.PostAsync("t91104/proc", new StringContent(JsonConvert.SerializeObject(SelectedMstItems), System.Text.Encoding.UTF8, "application/json")))
                    {
                        if (response.IsSuccessStatusCode)
                        {
            ...
            {
                WinUIMessageBox.Show("[외주 불량 반납] 대상이 존재 하지 않습니다", _title, MessageBoxButton.OK, MessageBoxImage.Information, MessageBoxResult.None, MessageBoxOptions.None);
                return;
            }
        }
        ...
        [Command]
        public async void OpenReportViewContact(object _obj = null)
        {
            isPRNT_PLN = true;
        ...
        [Command]
        public async void BadRefresh(object _obj)
        {
            try
            {
                //TecVo Dao = _obj as TecVo;
                //if (Dao == null)
                //{
                //    return;
                //}

                //if (this.isPRNT_PLN == true)
                //{
                //    this.PRNT_PLN_TITLE = "불량 이력 - [" + Dao.ROUT_NM + "] " + Dao.GRP_SER_NO;

                //    using (HttpResponseMessage response = await SystemProperties.PROGRAM_HTTP.PostAsync("t91100/bad", new StringContent(JsonConvert.SerializeObject(Dao), System.Text.Encoding.UTF8, "application/json")))
                //    {
                //        if (response.IsSuccessStatusCode)
                //        {
                //            this.SelectedPopupViewList = JsonConvert.DeserializeObject<IEnumerable<TecVo>>(await response.Content.ReadAsStringAsync()).Cast<TecVo>().ToList();
                //        }
                //    }
                //}
            }
        ...
        public async void BarPrint()
        {
            ...
            //        //MessageBoxResult result = WinUIMessageBox.Show("[ 총 :" + SelectedMstItems.Count + " ] 정말로 바코드 하시겠습니까?", "[바코드]" + _title, MessageBoxButton.YesNo, MessageBoxImage.Question);
            //        //if (result == MessageBoxResult.Yes)
            //        //{
            //        //    DXSplashScreen.Show<ProgressWindow>();
            ...
            //        //        labelDao.A15 = "원료 / 자재 / 색소베이스 / 벌크제품 / 완제품";

            //        //        //바코드
            //        //        labelDao.A05 = _item.LOT_NO;
            //        //        //벌크 출력
            //        //        _bar.Bulk_Godex(labelDao);
            //        //    }
        ...
        private string _PRNT_PLN_TITLE;
        public string PRNT_PLN_TITLE
        {
            get { return _PRNT_PLN_TITLE; }
            set { SetProperty(ref _PRNT_PLN_TITLE, value, () => PRNT_PLN_TITLE); }
        }
