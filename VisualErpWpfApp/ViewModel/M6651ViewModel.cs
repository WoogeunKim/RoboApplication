...
{
    public sealed class M6651ViewModel : ViewModelBase, INotifyPropertyChanged
    {
        ...
        private M6651MasterDialog masterDialog;
        ...
        [Command]
        public async void Refresh(string _PROD_PLN_NO = null)
        {
            try
            {
                using (HttpResponseMessage response = await SystemProperties.PROGRAM_HTTP.PostAsync("m6651", new StringContent(JsonConvert.SerializeObject(new ManVo() { FM_DT = Convert.ToDateTime(StartDt).ToString("yyyyMM"), CHNL_CD = SystemProperties.USER_VO.CHNL_CD }), System.Text.Encoding.UTF8, "application/json")))
                {
                    ...
                    if (SelectMstList.Count >= 1)
                    {
                        ...
                        {
                            SelectedMstItem = SelectMstList.Where(x => x.PROD_PLN_NO.Equals(_PROD_PLN_NO)).LastOrDefault<ManVo>();
                        }
                    }
                    else
                    {
        ...
        {
            masterDialog = new M6651MasterDialog(new ManVo() {  PROD_PLN_QTY = 0 });
        ...
        public void EditDtlContact()
        {
            ...
            masterDialog = new M6651MasterDialog(SelectedMstItem);
            masterDialog.Title = title + " - 수정(" + SelectedMstItem.PROD_PLN_NO + "/" + SelectedMstItem.N1ST_ITM_GRP_NM + ")";
            masterDialog.Owner = Application.Current.MainWindow;
            masterDialog.BorderEffect = BorderEffect.Default;
            masterDialog.BorderEffectActiveColor = new SolidColorBrush(Color.FromRgb(255, 128, 0));
            masterDialog.BorderEffectInactiveColor = new SolidColorBrush(Color.FromRgb(255, 170, 170));
        ...
        public async void DelContact()
        {
            try
            {
                ...
                if (SelectedMstItem != null)
                {
                    MessageBoxResult result = WinUIMessageBox.Show("[" + SelectedMstItem.PROD_PLN_NO + "/" + SelectedMstItem.ITM_NM + "] 정말로 삭제 하시겠습니까?", title, MessageBoxButton.YesNo, MessageBoxImage.Question);
                    if (result == MessageBoxResult.Yes)
                    {
                        using (HttpResponseMessage response = await SystemProperties.PROGRAM_HTTP.PostAsync("m6651/d", new StringContent(JsonConvert.SerializeObject(SelectedMstItem), System.Text.Encoding.UTF8, "application/json")))
                        {
                            if (response.IsSuccessStatusCode)
                            {
                                int _Num = 0;
