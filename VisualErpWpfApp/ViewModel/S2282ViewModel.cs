...
{
    public sealed class S2282ViewModel : ViewModelBase, INotifyPropertyChanged
    {
        ...
        public S2282ViewModel() 
        {
            StartDt = System.DateTime.Now.AddYears(-1);
        ...
        public async void Refresh()
        {
            using (HttpResponseMessage response = await SystemProperties.PROGRAM_HTTP.PostAsync("s2282/mst", new StringContent(JsonConvert.SerializeObject(new SaleVo() { FM_DT = (StartDt).ToString("yyyy-MM-dd"), TO_DT = (EndDt).ToString("yyyy-MM-dd"), SL_AREA_CD = M_SL_AREA_NM.CLSS_CD, CHNL_CD = SystemProperties.USER_VO.CHNL_CD }), System.Text.Encoding.UTF8, "application/json")))
            {
                ...
                //SelectMstList = saleOrderClient.S2282SelectMstList(new JobVo() { FM_DT = (StartDt).ToString("yyyy-MM-dd"), TO_DT = (EndDt).ToString("yyyy-MM-dd"), SL_AREA_CD = (string.IsNullOrEmpty(TXT_SL_AREA_NM) ? null : _AreaMap[TXT_SL_AREA_NM]) });
                ////
