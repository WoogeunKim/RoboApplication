...
{
    public partial class M66212MasterDialog : DXWindow
    {
        ...
        public M66212MasterDialog(ManVo Dao)
        {
            InitializeComponent();
        ...
        private async void OKButton_Click(object sender, RoutedEventArgs e)
        {
            if (ValueCheckd())
            {
                ...
                using (HttpResponseMessage response = await SystemProperties.PROGRAM_HTTP.PostAsync("m66212/mst/d", new StringContent(JsonConvert.SerializeObject(new ManVo() { CHNL_CD = SystemProperties.USER_VO.CHNL_CD, ITM_CD = this.orgDao.ITM_CD }), System.Text.Encoding.UTF8, "application/json")))
                {
                ...
                using (HttpResponseMessage response = await SystemProperties.PROGRAM_HTTP.PostAsync("m66212/mst/i/list", new StringContent(JsonConvert.SerializeObject(this.totalItem), System.Text.Encoding.UTF8, "application/json")))
                {
                    if (response.IsSuccessStatusCode)
                    {
        ...
        public async void SYSTEM_CODE_VO()
        {
            ...
            using (HttpResponseMessage responseX = await SystemProperties.PROGRAM_HTTP.PostAsync("s141", new StringContent(JsonConvert.SerializeObject(new SystemCodeVo() { CHNL_CD = SystemProperties.USER_VO.CHNL_CD /*N1ST_ITM_GRP_CD = SelectedMasterItem.N1ST_ITM_GRP_CD*/ }), System.Text.Encoding.UTF8, "application/json")))
            {
                if (responseX.IsSuccessStatusCode)
                {
