...
namespace AquilaErpWpfApp3.View.TEC.Dialog
{
    ...
    /// </summary>
    public partial class T8814MasterDialog : DXWindow
    {
        ...
        public T8814MasterDialog(IList<TecVo> _chekList)
        {
            InitializeComponent();
            //
            //this.orgDao = Dao;
        ...
        private async void OKButton_Click(object sender, RoutedEventArgs e)
        {
            if (ValueCheckd())
            {
                ...
                //{
                //this.updateDao = getDomain();
                SystemCodeVo _itmNm = this.combo_ITM_NM.SelectedItem as SystemCodeVo;
                ...
                foreach (TecVo item in this.ChekList)
                {
                    ...
                    if (_itmNm != null)
                    {
                        item.ITM_CD = _itmNm.ITM_CD;
                        item.ITM_NM = _itmNm.ITM_NM;
                    }
                ...
                //
                //
                using (HttpResponseMessage response = await SystemProperties.PROGRAM_HTTP.PostAsync("t8814/m", new StringContent(JsonConvert.SerializeObject(this.ChekList), System.Text.Encoding.UTF8, "application/json")))
                {
                    if (response.IsSuccessStatusCode)
                    {
                        string result = await response.Content.ReadAsStringAsync();
        ...
        public Boolean ValueCheckd()
        {
            ...
            if (string.IsNullOrEmpty(this.text_MTRL_EXP_DY.Text))
            {
                //UOM_CD = new CodeDao() { CLSS_DESC = ITM_NM.UOM_NM };
                //MTRL_MAKE_DT = Convert.ToDateTime(MTRL_MAKE_DT).AddDays(MTRL_EXP_DY).ToString("yyyy-MM-dd");
                WinUIMessageBox.Show("[사용기한(일수)] 입력 값이 맞지 않습니다", "[유효검사] " + this._title, MessageBoxButton.OK, MessageBoxImage.Warning);
        ...
        {
            //제품명
            using (HttpResponseMessage response = await SystemProperties.PROGRAM_HTTP.PostAsync("s141/mini", new StringContent(JsonConvert.SerializeObject(new SystemCodeVo() { ITM_GRP_CLSS_CD = "G", CHNL_CD = SystemProperties.USER_VO.CHNL_CD, DELT_FLG = "N" }), System.Text.Encoding.UTF8, "application/json")))
            {
                if (response.IsSuccessStatusCode)
                {
                    List<SystemCodeVo> _list = JsonConvert.DeserializeObject<IEnumerable<SystemCodeVo>>(await response.Content.ReadAsStringAsync()).Cast<SystemCodeVo>().ToList();

                    this.combo_ITM_NM.ItemsSource = _list;
                    this.combo_ITM_NM.SelectedItem = _list.Where<SystemCodeVo>(x => x.ITM_CD.Equals(this.ChekList[0].ITM_CD)).FirstOrDefault<SystemCodeVo>();
