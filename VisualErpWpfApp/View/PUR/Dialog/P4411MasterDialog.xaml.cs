...
{
    public partial class P4411MasterDialog : DXWindow
    {
        ...
        public P4411MasterDialog(PurVo Dao)
        {
            InitializeComponent();
        ...
        private async void OKButton_Click(object sender, RoutedEventArgs e)
        {
            if (ValueCheckd())
            {
                ...
                if (isEdit == false)
                {
                    ...
                    using (HttpResponseMessage response = await SystemProperties.PROGRAM_HTTP.PostAsync("p4411/mst/i", new StringContent(JsonConvert.SerializeObject(this.updateDao), System.Text.Encoding.UTF8, "application/json")))
                    {
                ...
                else
                {
                    ...
                    using (HttpResponseMessage response = await SystemProperties.PROGRAM_HTTP.PostAsync("p4411/mst/u", new StringContent(JsonConvert.SerializeObject(this.updateDao), System.Text.Encoding.UTF8, "application/json")))
                    {
                        if (response.IsSuccessStatusCode)
                        {
                            string result = await response.Content.ReadAsStringAsync();
        ...
        public Boolean ValueCheckd()
        {
            ...
            {
                WinUIMessageBox.Show("[발주 일자] 입력 값이 맞지 않습니다.", "[유효검사]발주 등록 관리", MessageBoxButton.OK, MessageBoxImage.Warning);
