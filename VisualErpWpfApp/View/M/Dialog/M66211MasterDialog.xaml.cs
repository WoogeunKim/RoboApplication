    ...
    public partial class M66211MasterDialog : DXWindow
    {
        ...
        public M66211MasterDialog(ManVo Dao)
        {
            ...
            ManVo copyDao = new ManVo()
            {
                SENS_CD = Dao.SENS_CD,
                SENS_NM = Dao.SENS_NM,
                SENS_BSE_VAL = Dao.SENS_BSE_VAL,
                SENS_TOL_VAL = Dao.SENS_TOL_VAL,
            ...
            //수정
            if (copyDao.SENS_CD != null)
            {
                this.text_SENS_CD.IsReadOnly = true;
                this.isEdit = true;
            }
            else
            {
                ...
                //this.isEdit = false;
                copyDao.SENS_BSE_VAL = 0;
                copyDao.SENS_TOL_VAL = 0;
                copyDao.DELT_FLG = "사용";
            }
        ...
        private async void OKButton_Click(object sender, RoutedEventArgs e)
        {
            if (ValueCheckd())
            {
                ...
                if (isEdit == false)
                {
                    ...
                    using (HttpResponseMessage response = await SystemProperties.PROGRAM_HTTP.PostAsync("m66211/i", new StringContent(JsonConvert.SerializeObject(this.updateDao), System.Text.Encoding.UTF8, "application/json")))
                    {
                ...
                else
                {
                    ...
                    using (HttpResponseMessage response = await SystemProperties.PROGRAM_HTTP.PostAsync("m66211/u", new StringContent(JsonConvert.SerializeObject(this.updateDao), System.Text.Encoding.UTF8, "application/json")))
                    {
                        if (response.IsSuccessStatusCode)
                        {
                            string result = await response.Content.ReadAsStringAsync();
        ...
        {
            if (string.IsNullOrEmpty(this.text_SENS_CD.Text))
            {
                WinUIMessageBox.Show("[코드] 입력 값이 맞지 않습니다.", "[유효검사]" + _title, MessageBoxButton.OK, MessageBoxImage.Warning);
                this.text_SENS_CD.IsTabStop = true;
                this.text_SENS_CD.Focus();
                return false;
            }
        ...
        {
            ManVo Dao = new ManVo();
            Dao.SENS_CD = this.text_SENS_CD.Text;
            Dao.SENS_NM = this.text_SENS_NM.Text;
            Dao.SENS_BSE_VAL = this.text_SENS_BSE_VAL.Text;
