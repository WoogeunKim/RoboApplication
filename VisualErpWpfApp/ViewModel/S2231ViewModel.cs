...
{
    public sealed class S2231ViewModel : ViewModelBase, INotifyPropertyChanged
    {
        ...
        private S2231MasterDialog masterDialog;
        ...
        [Command]
        public async void Refresh(string _BANK_ACCT_CD = null)
        {
            try
            {
                using (HttpResponseMessage response = await SystemProperties.PROGRAM_HTTP.PostAsync("s2231", new StringContent(JsonConvert.SerializeObject(new SaleVo() { CHNL_CD = SystemProperties.USER_VO.CHNL_CD }), System.Text.Encoding.UTF8, "application/json")))
                {
                    ...
                    if (SelectMstList.Count >= 1)
                    {
                        ...
                        if (string.IsNullOrEmpty(_BANK_ACCT_CD))
                        {
                            SelectedMstItem = SelectMstList[0];
                        }
                        else
                        {
                            SelectedMstItem = SelectMstList.Where(x => x.BANK_ACCT_CD.Equals(_BANK_ACCT_CD)).LastOrDefault<SaleVo>();
                        }
                    }
                    else
                    {
        ...
        {
            masterDialog = new S2231MasterDialog(new SaleVo());
            ...
            if (isDialog)
            {
                Refresh(masterDialog.ResultDao.BANK_ACCT_CD);
            }
        }
        ...
        public void EditDtlContact()
        {
            ...
            masterDialog = new S2231MasterDialog(SelectedMstItem);
            masterDialog.Title = title + " - 수정(" + SelectedMstItem.BANK_ACCT_CD + " / " + SelectedMstItem.BANK_ACCT_NM + ")";
            masterDialog.Owner = Application.Current.MainWindow;
            masterDialog.BorderEffect = BorderEffect.Default;
            masterDialog.BorderEffectActiveColor = new SolidColorBrush(Color.FromRgb(255, 128, 0));
            masterDialog.BorderEffectInactiveColor = new SolidColorBrush(Color.FromRgb(255, 170, 170));
        ...
        public async void DelContact()
        {
            try
            {
                ...
                if (SelectedMstItem != null)
                {
                    MessageBoxResult result = WinUIMessageBox.Show(SelectedMstItem.BANK_ACCT_NM + " 정말로 삭제 하시겠습니까?", title, MessageBoxButton.YesNo, MessageBoxImage.Question);
                    if (result == MessageBoxResult.Yes)
                    {
                        using (HttpResponseMessage response = await SystemProperties.PROGRAM_HTTP.PostAsync("s2231/d", new StringContent(JsonConvert.SerializeObject(SelectedMstItem), System.Text.Encoding.UTF8, "application/json")))
                        {
                            if (response.IsSuccessStatusCode)
                            {
                                int _Num = 0;
