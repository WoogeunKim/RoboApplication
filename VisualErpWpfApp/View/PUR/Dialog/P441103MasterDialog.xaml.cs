...
{
    public partial class P441103MasterDialog : DXWindow
    {
        ...
        public P441103MasterDialog(PurVo Dao)
        {
            InitializeComponent();
        ...
        private async void OKButton_Click(object sender, RoutedEventArgs e)
        {
            if (ValueCheckd())
            {
                ...
                if (isEdit == false)
                {
                    ...
                    using (HttpResponseMessage response = await SystemProperties.PROGRAM_HTTP.PostAsync("p441102/mst/i", new StringContent(JsonConvert.SerializeObject(this.updateDao), System.Text.Encoding.UTF8, "application/json")))
                    {
                ...
                else
                {
                    ...
                    using (HttpResponseMessage response = await SystemProperties.PROGRAM_HTTP.PostAsync("p441102/mst/u", new StringContent(JsonConvert.SerializeObject(this.updateDao), System.Text.Encoding.UTF8, "application/json")))
                    {
                        if (response.IsSuccessStatusCode)
                        {
                            string result = await response.Content.ReadAsStringAsync();
        ...
        public Boolean ValueCheckd()
        {
            ...
            {
                WinUIMessageBox.Show("[발주 일자] 입력 값이 맞지 않습니다.", "[유효검사]원자재 발주서 등록", MessageBoxButton.OK, MessageBoxImage.Warning);
            ...
            {
                WinUIMessageBox.Show("[매입처] 입력 값이 맞지 않습니다.", "[유효검사]원자재 발주서 등록", MessageBoxButton.OK, MessageBoxImage.Warning);
            ...
            {
                WinUIMessageBox.Show("[발주 품목] 입력 값이 맞지 않습니다.", "[유효검사]원자재 발주서 등록", MessageBoxButton.OK, MessageBoxImage.Warning);
        ...
        private PurVo getDomain()
        {
            ...
            if (string.IsNullOrEmpty(this.combo_PUR_WK_CD.Text) == false)
            {
