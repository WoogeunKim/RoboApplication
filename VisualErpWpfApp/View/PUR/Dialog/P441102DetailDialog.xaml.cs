...
{
    public partial class P441102DetailDialog : DXWindow
	{
        ...
        private string _title = "원자재 발주 등록";
        ...
        public P441102DetailDialog(PurVo Dao)
		{
			InitializeComponent();
       ...
       //         }
                ...
                using (HttpResponseMessage response = await SystemProperties.PROGRAM_HTTP.PostAsync("p441102/popup", new StringContent(JsonConvert.SerializeObject(this.orgDao), System.Text.Encoding.UTF8, "application/json")))
                {
                    if (response.IsSuccessStatusCode)
                    {
		...
		private async void OKButton_Click(object sender, RoutedEventArgs e)
		{
            try
            {
                ...
                {
                    int _Num = 0;
                    using (HttpResponseMessage response = await SystemProperties.PROGRAM_HTTP.PostAsync("p441102/dtl/i", new StringContent(JsonConvert.SerializeObject((saveItems.FindAll(x => x.isCheckd == true)).ToArray()), System.Text.Encoding.UTF8, "application/json")))
                    {
                        if (response.IsSuccessStatusCode)
                        {
