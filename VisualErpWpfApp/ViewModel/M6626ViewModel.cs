...
{
    //계측기 관리
    public sealed class M6626ViewModel : ViewModelBase, INotifyPropertyChanged
    {
        ...
        //
        private M6626MasterDialog masterDialog;


        public M6626ViewModel() 
        {
            Refresh();
        }
        ...
        public async void Refresh(string _INSRT_MGMT_NO = null)
        {
            try
            {
                using (HttpResponseMessage response = await SystemProperties.PROGRAM_HTTP.PostAsync("m6626", new StringContent(JsonConvert.SerializeObject(new ManVo() { CHNL_CD = SystemProperties.USER_VO.CHNL_CD }), System.Text.Encoding.UTF8, "application/json")))
                {
                    ...
                    //SelectedMasterViewList = manClient.M6626SelectMaster(new ManVo() { CHNL_CD = SystemProperties.USER_VO.CHNL_CD });
        ...
        {
            masterDialog = new M6626MasterDialog(new ManVo());
        ...
        public void EditMasterContact()
        {
            ...
            if (editDao != null)
            {
                masterDialog = new M6626MasterDialog(editDao);
        ...
        public async void DelMasterContact()
        {
            try
            {
                ...
                if (delDao != null)
                {
                    MessageBoxResult result = WinUIMessageBox.Show(delDao.INSRT_MGMT_NO + "/" + delDao.INSRT_NM + " 정말로 삭제 하시겠습니까?", _title , MessageBoxButton.YesNo, MessageBoxImage.Question);
                    if (result == MessageBoxResult.Yes)
                    {
                        using (HttpResponseMessage response = await SystemProperties.PROGRAM_HTTP.PostAsync("m6626/d", new StringContent(JsonConvert.SerializeObject(delDao), System.Text.Encoding.UTF8, "application/json")))
                        {
                            if (response.IsSuccessStatusCode)
                            {
                                int _Num = 0;
