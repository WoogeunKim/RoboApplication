...
{
    //표준 공정 관리
    public sealed class M6611ViewModel : ViewModelBase, INotifyPropertyChanged
    {
        ...
        public M6611ViewModel() 
        {
            Refresh();
        }

        [Command]
       public async void Refresh(string _ROUT_CD = null)
        {
            try
            {
                using (HttpResponseMessage response = await SystemProperties.PROGRAM_HTTP.PostAsync("m6611", new StringContent(JsonConvert.SerializeObject(new ManVo() { CHNL_CD = SystemProperties.USER_VO.CHNL_CD }), System.Text.Encoding.UTF8, "application/json")))
                {
                    ...
                    if (SelectedMasterViewList.Count > 0)
                    {
                        ...
                        if (string.IsNullOrEmpty(_ROUT_CD))
                        {
                            SelectedMasterItem = SelectedMasterViewList[0];
                        }
                        else
                        {
                            SelectedMasterItem = SelectedMasterViewList.Where(x => x.ROUT_CD.Equals(_ROUT_CD)).LastOrDefault<ManVo>();
                        }
                    }
                    else
                    {
        ...
        {
            masterDialog = new M6611MasterDialog(new ManVo());
        ...
        public async void DelMasterContact()
        {
            try
            {
                ...
                if (delDao != null)
                {
                    ...
                    {
                        using (HttpResponseMessage response = await SystemProperties.PROGRAM_HTTP.PostAsync("m6611/d", new StringContent(JsonConvert.SerializeObject(delDao), System.Text.Encoding.UTF8, "application/json")))
                        {
                            if (response.IsSuccessStatusCode)
                            {
                                int _Num = 0;
