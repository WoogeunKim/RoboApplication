...
{
    public sealed class I66110ViewModel : ViewModelBase, INotifyPropertyChanged
    {
        ...
        private I66110DetailPurDialog detailPurDialog;
        private I66110DetailOthDialog detailOthDialog;
       ...
       public async void Refresh()
        ...
        public void NewDtlPurContact()
        {
            try
            {
                detailPurDialog = new I66110DetailPurDialog(new InvVo() {CHNL_CD = SystemProperties.USER_VO.CHNL_CD, FM_DT = (StartDt).ToString("yyyy-MM-dd"), TO_DT = (EndDt).ToString("yyyy-MM-dd") });
                detailPurDialog.Title = "발주 입고";
        ...
        public async void BarPrint()
        {
            try
            {
                ...
                {
                    MessageBoxResult result = WinUIMessageBox.Show("[" + SelectedMstItem.INSRL_NO + "] 정말로 바코드 하시겠습니까?", "[바코드 - 출력]" + _title, MessageBoxButton.YesNo, MessageBoxImage.Question);
                    if (result == MessageBoxResult.Yes)
                    {
                        // DXSplashScreen.Show<ProgressWindow>();
                        ...
                        //{
                        I66110BarCodeReport BarCodeReport = new I66110BarCodeReport(new List<InvVo>() { SelectedMstItem });
