...
{
    //기타 출고
    public partial class I5512DetailEtcDialog : DXWindow
    {
        ...
        public I5512DetailEtcDialog(InvVo vo)
        {
            InitializeComponent();
        ...
        private async void searchItem()
        {
            try
            {
                ...
                using (HttpResponseMessage response = await SystemProperties.PROGRAM_HTTP.PostAsync("i5512/popup/etc", new StringContent(JsonConvert.SerializeObject(vo), System.Text.Encoding.UTF8, "application/json")))
                {
                ...
                //this.ViewJOB_ITEMEdit.ItemsSource = invClient.I5511SelectDtlEtcOutList(vo);
        ...
        public async void SYSTEM_CODE_VO()
        {
            ...
            using (HttpResponseMessage response = await SystemProperties.PROGRAM_HTTP.GetAsync("s131/dtl/" + Properties.Settings.Default.SettingChnl + "/" + "L-007"))
            {
                if (response.IsSuccessStatusCode)
                {
                    this.lue_INAUD_NM.ItemsSource = JsonConvert.DeserializeObject<IEnumerable<SystemCodeVo>>(await response.Content.ReadAsStringAsync()).Cast<SystemCodeVo>().Where<SystemCodeVo>(x => x.CLSS_CD.StartsWith("I")).ToList();
            ...
            // this.lue_CO_NM.ItemsSource = SystemProperties.CUSTOMER_CODE_VO(null, null);
            using (HttpResponseMessage response = await SystemProperties.PROGRAM_HTTP.PostAsync("s143", new StringContent(JsonConvert.SerializeObject(new SystemCodeVo() { DELT_FLG = "N", SEEK_AR = "AR", SEEK = "AR",  CO_TP_CD = "AR", AREA_CD = SystemProperties.USER_VO.EMPE_PLC_NM, CHNL_CD = SystemProperties.USER_VO.CHNL_CD }), System.Text.Encoding.UTF8, "application/json")))
            {
                if (response.IsSuccessStatusCode)
                {
