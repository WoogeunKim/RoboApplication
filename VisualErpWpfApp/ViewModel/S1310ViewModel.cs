...
{
    public sealed class S1310ViewModel : ViewModelBase, INotifyPropertyChanged
    {
        private string title = "게시판";
        ...
        public async void Refresh(string _TIT_DESC = null)
        {
            try
            {
                ...
                using (HttpResponseMessage response = await SystemProperties.PROGRAM_HTTP.PostAsync("s1310", new StringContent(JsonConvert.SerializeObject(new SystemCodeVo() { FM_DT = (StartDt).ToString("yyyy-MM-dd"), TO_DT = (EndDt).ToString("yyyy-MM-dd"), CHNL_CD = SystemProperties.USER_VO.CHNL_CD, NTC_CLSS_CD = "A", NTC_CD = "A" }), System.Text.Encoding.UTF8, "application/json")))
                {
        ...
        public void NewContact()
        {
            ...
            masterDialog = new S1310MasterDialog(new SystemCodeVo() { CRE_DT = System.DateTime.Now.ToString("yyyy-MM-dd"), INP_ID = SystemProperties.USER_VO.USR_ID, INP_NM = SystemProperties.USER_VO.USR_N1ST_NM, CHNL_CD = SystemProperties.USER_VO.CHNL_CD, NTC_SEQ = 1, NTC_CLSS_CD = "A", NTC_CD = "A" });
