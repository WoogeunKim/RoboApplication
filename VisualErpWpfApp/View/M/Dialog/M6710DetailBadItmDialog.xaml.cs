...
namespace AquilaErpWpfApp3.View.M.Dialog
{
    ...
    /// </summary>
    public partial class M6710DetailBadItmDialog : DXWindow
    {
        ...
        private string title = "투입 자재 불량 현황";

        public M6710DetailBadItmDialog(ManVo Dao)
        {
            InitializeComponent();
        ...
        private async void OKButton_Click(object sender, RoutedEventArgs e)
        {
            try
            {
                ...
                //삭제
                using (HttpResponseMessage response = await SystemProperties.PROGRAM_HTTP.PostAsync("m6710/dtl3/d", new StringContent(JsonConvert.SerializeObject(this.orgDao), System.Text.Encoding.UTF8, "application/json")))
                {
                ...
                {
                    //저장
                    using (HttpResponseMessage response = await SystemProperties.PROGRAM_HTTP.PostAsync("m6710/dtl3/m", new StringContent(JsonConvert.SerializeObject(this.totalItem), System.Text.Encoding.UTF8, "application/json")))
                    {
                        if (response.IsSuccessStatusCode)
                        {
                            int _Num = 0;
        ...
        {
            //투입자재 불량 현황
            using (HttpResponseMessage response = await SystemProperties.PROGRAM_HTTP.PostAsync("m6710/dtl3/dtl", new StringContent(JsonConvert.SerializeObject(this.orgDao), System.Text.Encoding.UTF8, "application/json")))
            {
                if (response.IsSuccessStatusCode)
                {
