...
{
    public sealed class S1146ViewModel : ViewModelBase, INotifyPropertyChanged
    {
        private string title = "제품단가등록(내부2)";
        ...
        private S1146MasterDialog masterDialog;
        ...
        public async void Refresh(string _ITM_CD = null)
        {
            try
            {
                ...
                using (HttpResponseMessage response = await SystemProperties.PROGRAM_HTTP.PostAsync("s1146", new StringContent(JsonConvert.SerializeObject(_param), System.Text.Encoding.UTF8, "application/json")))
                {
        ...
        public void NewContact()
        {
            ...
            masterDialog = new S1146MasterDialog(new SystemCodeVo() { AREA_CD = M_SL_AREA_VO.CLSS_CD, AREA_NM = M_SL_AREA_VO.CLSS_DESC, ITM_GRP_CLSS_NM = M_ITM_GRP_CLSS_VO.CLSS_DESC, ITM_GRP_CLSS_CD = M_ITM_GRP_CLSS_VO.CLSS_CD });
            masterDialog.Title = title + " - 추가 (사업장)";
        ...
        public void EditContact()
        {
            ...
            //    //string itmCd = SelectedMstItem.ITM_CD;
            masterDialog = new S1146MasterDialog(SelectedMstItem);
            masterDialog.Title = title + " - 수정 (사업장)";
            masterDialog.Owner = Application.Current.MainWindow;
            masterDialog.BorderEffect = BorderEffect.Default;
            masterDialog.BorderEffectActiveColor = new SolidColorBrush(Color.FromRgb(255, 128, 0));
            masterDialog.BorderEffectInactiveColor = new SolidColorBrush(Color.FromRgb(255, 170, 170));
            ...
            if (SelectedMstItem != null)
            {
                ...
                if (result == MessageBoxResult.Yes)
                {
                    try
                    {
                        ...
                        using (HttpResponseMessage response = await SystemProperties.PROGRAM_HTTP.PostAsync("S1146/d", new StringContent(JsonConvert.SerializeObject(SelectedMstItem), System.Text.Encoding.UTF8, "application/json")))
                        {
                            if (response.IsSuccessStatusCode)
                            {
                                int _Num = 0;
        ...
        public async void SYSTEM_CODE_VO()
        {
            ...
            using (HttpResponseMessage response = await SystemProperties.PROGRAM_HTTP.GetAsync("s131/dtl/" + Properties.Settings.Default.SettingChnl + "/" + "L-001"))
            {
                if (response.IsSuccessStatusCode)
                {
                    ...
                    {
                        M_ITM_GRP_CLSS_VO = ITM_GRP_CLSS_LIST[2];
                    }
                }
            }
