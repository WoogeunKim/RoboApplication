...
namespace AquilaErpWpfApp3.View.S.Dialog
{
    ...
    /// </summary>
    public partial class S1144DetailDialog : DXWindow
    {
        ...
        private string title = "창고 위치 관리";

        public S1144DetailDialog(SystemCodeVo Dao)
        {
            InitializeComponent();
        ...
        async void btn_del_Click(object sender, RoutedEventArgs e)
        {
            ...
            if (result == MessageBoxResult.Yes)
            {
                try
                {
                    using (HttpResponseMessage response = await SystemProperties.PROGRAM_HTTP.PostAsync("s1144/mst/d", new StringContent(JsonConvert.SerializeObject(new SystemCodeVo() { LOC_SEQ = delVo.LOC_SEQ, ITM_CD = this.orgDao.ITM_CD, AREA_CD = SystemProperties.USER_VO.EMPE_PLC_CD, CRE_USR_ID = SystemProperties.USER, UPD_USR_ID = SystemProperties.USER, CHNL_CD = SystemProperties.USER_VO.CHNL_CD}), System.Text.Encoding.UTF8, "application/json")))
                    {
        ...
        async void btn_add_Click(object sender, RoutedEventArgs e)
        {
            ...
            using (HttpResponseMessage response = await SystemProperties.PROGRAM_HTTP.PostAsync("s1144/mst/i", new StringContent(JsonConvert.SerializeObject(new SystemCodeVo() { LOC_SEQ = this.totalItem.Count + 1, ITM_CD = this.orgDao.ITM_CD, AREA_CD = SystemProperties.USER_VO.EMPE_PLC_NM, CRE_USR_ID = SystemProperties.USER, UPD_USR_ID = SystemProperties.USER, N1ST_LOC_ID = N1ST_LOC_NM.CLSS_CD, N1ST_LOC_NM = N1ST_LOC_NM.CLSS_DESC, N2ND_LOC_ID = N2ND_LOC_NM.CLSS_CD, N2ND_LOC_NM = N2ND_LOC_NM.CLSS_DESC, N3RD_LOC_ID = N3RD_LOC_NM.CLSS_CD, N3RD_LOC_NM = N3RD_LOC_NM.CLSS_DESC, LOC_NM = N1ST_LOC_NM.CLSS_DESC + N2ND_LOC_NM.CLSS_DESC + "-" + N3RD_LOC_NM.CLSS_DESC, CHNL_CD = SystemProperties.USER_VO.CHNL_CD }), System.Text.Encoding.UTF8, "application/json")))
            {
                if (response.IsSuccessStatusCode)
                {
                    int _Num = 0;
        ...
        {
            using (HttpResponseMessage response = await SystemProperties.PROGRAM_HTTP.PostAsync("s1144/dtl", new StringContent(JsonConvert.SerializeObject(this.orgDao), System.Text.Encoding.UTF8, "application/json")))
            {
                if (response.IsSuccessStatusCode)
                {
                ...
                if (n1stLocNm)
                {
                    if (e.IsValid)
                    {
                        ...
                        if (!masterDomain.N1ST_LOC_NM.Equals((e.Value == null ? "" : e.Value.ToString())))
                        {
                            ...
                            masterDomain.AREA_CD = this.orgDao.AREA_CD;
